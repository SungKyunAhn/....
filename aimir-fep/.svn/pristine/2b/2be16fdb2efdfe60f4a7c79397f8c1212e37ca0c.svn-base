 ...
 */
public class ST023 implements java.io.Serializable {

	private static final long serialVersionUID = -6111051239374181411L;
    ...
    private static Log log = LogFactory.getLog(ST023.class);
	...
	/**
	 * Constructor .<p>
	 * !!noted i210 programming guide ,while the meter will always transport 5 summation and 5demand only the first 2 of each will be valid.
	 * The remaining 3 summations and demands will always be transported as zeros when read.
	...
	public void parseData() throws Exception {
		...
		if(meterMode != null && meterMode.getCode() == MeterMode.DemandLp.getCode()){
			
			if(data.length < 241 && data.length > 0){
				this.NBR_SUM   = 2; 
				this.NBR_DMD   = 0;
				this.NBR_COIN  = 0;
			}
		}else if(meterMode != null && meterMode.getCode() == MeterMode.TOU.getCode()){
			if(data.length < 726 && data.length > 0){
				this.NBR_SUM   = 2; 
				this.NBR_DMD   = 0;
				this.NBR_COIN  = 0;
			}
