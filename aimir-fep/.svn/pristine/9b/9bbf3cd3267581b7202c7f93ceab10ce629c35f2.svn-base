...
import com.aimir.fep.bypass.decofactory.consts.DlmsConstantsForEVN.ImageTransferStatus;
...
import com.aimir.fep.bypass.decofactory.decorator.NestedDLMSDecoratorForEVN;
import com.aimir.fep.bypass.decofactory.decorator.NestedHDLCDecoratorForEVN;
 ...
 *
 */
public class BypassEVNFactory extends BypassFrameFactory {
	private static Logger logger = LoggerFactory.getLogger(BypassEVNFactory.class);
	...
	public BypassEVNFactory(String command) {
		this.command = command;
		frame = new NestedHDLCDecoratorForEVN(new NestedDLMSDecoratorForEVN(new EVN_DLMSFrame()));
	...
	public boolean start(IoSession session, Object type) {
		...
		} else if (command.equals("cmdTOUGet")) {
			
			// Procedure 설정
			int touProcedureIndex = 0;
			touProcedure = new Procedure[5];
			touProcedure[0] = Procedure.GET_CALENDAR_NAME_PASSIVE;
			touProcedure[1] = Procedure.GET_SEASON_PROFILE;
			touProcedure[2] = Procedure.GET_WEEK_PROFILE;
			touProcedure[3] = Procedure.GET_DAY_PROFILE;
			touProcedure[4] = Procedure.GET_STARTING_DATE;
...
//					break;
				...
				if (frame.isHDLCSegmented()) {
					...
					result = sendBypass(HdlcObjectType.RR);
					...
					if (command.equals("cmdMeterOTAStart")) {
						...
						case AARE:
							if (this.step == Procedure.HDLC_AARQ) {
								...
								logger.debug("## HDLC_AARQ Result => {}", param.name());
						...
						case GET_RES:
							...
							if (param instanceof DataAccessResult) {
								...
								Boolean isLast = true;
								HashMap<String, Object> tempParamMap = null;
								if (param instanceof HashMap) {
									tempParamMap = (HashMap<String, Object>)param;
									isBlock = ((Boolean)tempParamMap.get("isBlock") == null)? false : (Boolean)tempParamMap.get("isBlock");
									isLast = ((Boolean)tempParamMap.get("isLast") == null)? true : (Boolean)tempParamMap.get("isLast");
								}
								
								if(isBlock && !isLast) {
									this.setParam(tempParamMap);
									result = sendBypass();
								} else {
									logger.debug("## GET_RES step => {}", step.name());
									if(param != null) {
										logger.debug("## GET_RES addResultValue => {}", step.name());
										bypassFrameResult.addResultValue(step.name(), param);
										result = true;
									}
	...
	private boolean sendBypass(HdlcObjectType hdlcType) {
		boolean result = false;
		byte[] req = null;
		...
		try {
			...
			if (hdlcType != null) {
				if (hdlcType == HdlcObjectType.RR) {
					req = frame.encode(HdlcObjectType.RR, null, null);
					if (req != null) {
				...
				case GET_CALENDAR_NAME_PASSIVE:
					req = frame.encode(HdlcObjectType.GET_REQ, this.step, params);
					if (req != null) {
						logger.debug("### [{}] HDLC_REQUEST => {}", this.step.name(), Hex.decode(req));
						this.session.write(req);
						result = true;
	...
	public static void main(String[] args) {
		String command = "cmdGetMeterFWVersion";
		BypassEVNFactory factory = new BypassEVNFactory(command);
		factory.step = Procedure.GET_FIRMWARE_VERSION;
		factory.sendBypass();
	}
