 ...
 *
 */
public class EMnVMeterScanAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVMeterScanAction.class);
	private EMnVCommandSubFrameType commandType;

	public EMnVMeterScanAction(EMnVCommandSubFrameType commandType) {
	...
	public void execute(EMnVGeneralFramePayLoad gFramePayload, IoSession session, EMnVGeneralDataFrame generalDataFrame) throws Exception {
		...
		case COMMAND_REQUEST:
            ...
            logger.debug("[{}][COMMAND_REQUEST][TRAN_ID={}] - DEVICE={}, COMMAND={}, STATE={}, ASYNC_SIZE={}"
            		, new Object[]{ActionCommandType.METER_SCAN.name(), aDevice.getTransactionId(), aDevice.getModemId(), aDevice.getCommand(), TR_STATE.Waiting.getCode(), acllist.size ()});
            if (acllist.size() > 0) {
            	/*
                ...
                payLoad.setCommandType(ActionCommandType.METER_SCAN);
                ...
                logger.info("");
                logger.info("#### [METER_SCAN][COMMAND_REQUEST][TRAN_ID={}][remote={}] hdlc_address={}"
		...
		case COMMAND_RESPONSE:
			...
			case SUCCESS:
				logger.info("[METER_SCAN][COMMAND_RESPONSE][{}][TRNX_ID={}] MeterScan Success !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
			...
			case NOT_SUPPORTED_COMMAND:
				logger.error("[METER_SCAN][COMMAND_RESPONSE][{}][TRNX_ID={}] MeterScan Fail !!! - NOT_SUPPORTED_COMMAND ", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case NOT_ACCESS_USER:
				logger.error("[METER_SCAN][COMMAND_RESPONSE][{}][TRNX_ID={}] MeterScan Fail !!! - NOT_ACCESS_USER", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case FAIL:
				logger.error("[METER_SCAN][COMMAND_RESPONSE][{}][TRNX_ID={}] MeterScan Fail !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
	                for (int i = 0; i < acLogList.size(); i++) {
	                	AsyncCommandLog acl = acLogList.get(i);
