...
/**
 * {@link MRPClientProtocolHandler} implementation of FEP (AiMiR and Meter Protocol).
 * 
...
public class MRPClientProtocolHandler extends IoHandlerAdapter
{
    ...
    public void messageReceived(IoSession session, Object message )
    {
        try 
        {
            log.debug("messageReceived::["+Hex.getHexDump((byte[])message)+"]");
	...
	*/
    ...
    public Object getMessageFromEDMIMeter(IoSession session) 
    	throws MRPException, FMPACKTimeoutException, IOException
    {
    	...
    	try{
			...
			do {
				...
				future.awaitUninterruptibly(CONN_TIMEOUT * 1000);
				if (future.isRead()) {
					byte[] tmp = (byte[]) future.getMessage();
					bos.write(tmp);
					if(tmp!=null)
						isETX = (tmp[tmp.length-1]==ETX);
					
					tmp=null;
				} else {
					log.debug(" waitTime[" + CONN_TIMEOUT + "] timeout["
							+ CONN_TIMEOUT + "]");
					throw new FMPACKTimeoutException(" Msg Receive Timeout["
							+ CONN_TIMEOUT + "]");
				}
			...
			return result;
    	}catch(FMPACKTimeoutException e){
    		throw e;
    	} catch (IOException e) {
			throw e;
		}finally{
...
//        return totmsg;
        ...
        throws MRPException, FMPACKTimeoutException
    {
        ...
        for(int i = 0 ; i < retry && !atzOk; i++)
        {
            ...
            {  
            	...
            	message = new String((byte[])msg).trim();
	...
	throws MRPException, FMPACKTimeoutException
    {
    	try{
        	...
        	String message = new String((byte[])getMsg(session,35));
...
//        	}
     ...
     * @throws Exception
     */
    private void connectCircuit(IoSession session,CircuitTarget target, boolean meterConnect)
        throws MRPException, FMPACKTimeoutException
    {
        String ATZ = 
            FMPProperty.getProperty("protocol.circuit.command.ATZ","ATZ");
        ...
        for(int i = 0 ; i < retry ; i++)
        {
            ...
            try 
            {  
                message = getMsgs(session,circuitIdleTime,2);
                log.debug("["+i+"] initCircuit Read MSG["+message+"]");
                if(message != null) 
                {
                    if(message.toLowerCase().indexOf("ok") >= 0)
                    {
                    	log.debug("ATZ Return is OK: okcnt["+okcnt+"]!");
                        okcnt++;
                        if(okcnt >= 1)
                        {
                            atzOk = true;
                    ...
                    {
                        message = getMsgs(session,circuitIdleTime,3);
                        log.debug("initCircuit Read MSG["+message+"]");
                    }
                } 
            }catch(Exception ex) {}
        ...
        boolean retryATDT = true;
        int retryCnt = 3;
        int curTry=0;
        while(retryATDT){
        	curTry++;
        	log.info("\r\n\r\n["+curTry+"/"+retryCnt+"]"+ATDT+mobileId+"\r\n");
	        ...
	        if(message != null && message.toLowerCase().indexOf("connect") < 0) {
	            ...
	            if(message.toLowerCase().indexOf("carrier") >= 0 && curTry > retryCnt){
	                throw new MRPException(MRPError.ERR_NOCARRIER,"No carrier!");
	            }
	            if(message.toLowerCase().indexOf("busy") >= 0  && curTry > retryCnt){
	                throw new MRPException(MRPError.ERR_MODEMBUSY,"Modem is now busy!");
	            }
	            if(message.toLowerCase().indexOf("answer") >= 0  && curTry > retryCnt){
	                throw new MRPException(MRPError.ERR_NOANSWER,"No answer!");
	            }
	            if(message.toLowerCase().indexOf("dialtone") >= 0  && curTry > retryCnt){
	                throw new MRPException(MRPError.ERR_NODIALTONE,"No dialtone!");
	            }
	            if(message.toLowerCase().indexOf("error") >= 0  && curTry > retryCnt){
	                throw new MRPException(MRPError.ERR_MODEMERROR,"Modem error!");
	            }
	            else
	            {	if( curTry > retryCnt) {            	
	            		throw new MRPException(MRPError.ERR_TODIALMODEM_CLASS,"Dialing failed.");
	            	}else {
	            		log.error("ATDT Fail!!");
	            	}
	            }
        ...
        if(mcuType == McuType.MMIU){        	
        	userPass(session, target.getMobileId(),0,3);
        }else {
        	userAuthPass(session, mcuType);
        }        
     ...
     * @throws Exception
     */
    public void initCircuit(IoSession session,CircuitTarget target, boolean meterConnect) 
        throws Exception
    {
	 ...
	 * @return
	 */
	public int sendMsgUseFuture(IoSession session, IoBuffer buf) {
		// read 명령 사용 설정. (kskim)
