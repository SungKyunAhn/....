...
public class CRCUtil
{
    ...
    public static final byte[] crc_high = {
    ...
    public static final byte[] crc_low = {
    ...
    public static byte[] calculate_Xmodem_Ccitt(byte[] pt)
    {
        int crc = 0;
        for(int i=0; i < pt.length; i++)
            crc = (crc<<8) ^ crc16tab[((crc>>8) ^ pt[i])&0x00FF];
        return DataUtil.get2ByteToInt(crc);
    }
    
    /**
     * calculate zigbee CRC
     * @param pt
     * @param char crc
     * @return byte[] crcr
     */
    public static byte[] Calculate_ZigBee_Crc(byte[] pt, char crc)
    {					 
    	byte  high, low;
    	char j;
        ...
        for (int i=0; i< pt.length; i++)  {
            ...
            j = (char)(((high & 0xff) ^ (pt[i] & 0xff)) & 0xffff);
            high =(byte)( low ^ crc_high[(j & 0xffff)]);
            low = crc_low[j];
        }

        byte[] crcr = new byte[]{low, high};
        return crcr;
    }
     ...
     * @return byte[] crcr
     */
    public static byte[] Calculate_ZigBee_Crc(byte[] pt, byte[] crc)
    {
    	byte  high, low;
    	char j;
