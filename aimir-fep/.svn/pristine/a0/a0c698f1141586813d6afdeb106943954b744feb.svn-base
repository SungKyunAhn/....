...
public class SEVCD extends VCParser 
{
    ...
    public void parse(byte[] data) throws Exception
    {
        ...
        Double[] ch = new Double[]{new Double(cuv),
                                   new Double(cv),
                                   new Double(ct),
                                   new Double(cp),
                                   momentFlow};
        ...
        while(data.length>pos) {
            ...
            switch(vm_kind) {
            case VM_KIND_VCINFO:
                VM_SEVCDINFO vm_vcinfo = new VM_SEVCDINFO(DataUtil.select(data, pos, vm_length));
    ...
    public LinkedHashMap getData()
    {
        ...
        res.put("[Current data"+(titleIndex++)+"]", "");
        ...
        if(lpList != null && lpList.size() > 0){
            res.put("[Interval Data"+(titleIndex++)+"]", "");
        ...
        if(vmList != null && vmList.size() > 0){
            ...
            for(Iterator iter = vmList.iterator();iter.hasNext();)
            {
                ...
                {
                    res.put("[Volume Corrector Information"+(titleIndex++)+"]","");
                ...
                else if(o instanceof VM_DAYMAX) 
                {
                    ...
                    if(data != null && data.length > 0){
                        res.put("[Day Max Data"+(titleIndex++)+"]","");
                ...
                {                    
                    ...
                    if(data != null && data.length > 0){
                        res.put("[Month Max Data"+(titleIndex++)+"]","");
                        for(int i=0;i<data.length;i++) {
                ...
                }else if(o instanceof VM_MONTHSTAT) 
                {
                    ...
                    if(data != null && data.length > 0){
                        res.put("[Month Stat Data"+(titleIndex++)+"]","");
                        for(int i=0;i<data.length;i++) {
                            res.put(data[i].getYyyymm(),data[i].toString());
                        }
                    }
                ...
                {                    
                    ...
                    if(data != null && data.length > 0){
                        res.put("[Parameter Log"+(titleIndex++)+"]","");
                        for(int i=0;i<data.length;i++) {
    ...
    public void parse(byte[] data, double pulseConst) throws Exception
    {
        parse(data);
    }
