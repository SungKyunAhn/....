 ...
 *
 */
public class EMnVInverterSetupAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVInverterSetupAction.class);
	private EMnVCommandSubFrameType commandType;
	private RWSType rwsType;
	private ActionCommandType actionCommandType = ActionCommandType.INVERTER_SETUP;

	public EMnVInverterSetupAction(EMnVCommandSubFrameType commandType) {
            ...
            if (acllist.size() > 0) {
               ...
               String vendor = null;
                ...
                for(AsyncCommandParam param : acplist){
                	if(param.getParamType().equals("PORT_NUMBER")){
                		portNumber = param.getParamValue();
                	}else if(param.getParamType().equals("ENABLE")){
                		enable = param.getParamValue();
                	}else if(param.getParamType().equals("DEVICE_NUMBER")){
                		deviceNumber = param.getParamValue();
                	}else if(param.getParamType().equals("VENDOR")){
                		vendor = param.getParamValue();
                	}
                }
                ...
                payLoad.setInverterPortNumber(portNumber);
                payLoad.setInverterEnable(enable);
                payLoad.setInverterDeviceNumber(deviceNumber);
                payLoad.setInverterVendor(vendor);
                
                generalDataFrame.setGeneralFramePayLoad(payLoad);
                
                logger.info("");
                logger.info("#### [{}][COMMAND_REQUEST][TRAN_ID={}][remote={}] RWS_Type={}, PORT_NUMBER={}, ENABLE={}, DEVICE_NUMBER={}, VENDOR={}"
                		, new Object[]{actionCommandType.name(), aDevice.getTransactionId()
                				, session.getRemoteAddress(), rwsType, portNumber, enable, deviceNumber, vendor});
