 ...
 *
 */
public class EMnVMNumberAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVMNumberAction.class);
	private EMnVCommandSubFrameType commandType;
	private RWSType rwsType;
	private ActionCommandType actionCommandType = ActionCommandType.M_NUMBER;

	public EMnVMNumberAction(EMnVCommandSubFrameType commandType) {
	...
	public void execute(EMnVGeneralFramePayLoad gFramePayload, IoSession session, EMnVGeneralDataFrame generalDataFrame) throws Exception {
		...
		case COMMAND_RESPONSE:
			...
			case SUCCESS:
				String mNumber = DataUtil.getString(payLoad.getmNumber()).trim();
				logger.info("[{}][COMMAND_RESPONSE][{}][TRNX_ID={}][{}-{}] Success !!!"
						, new Object[]{actionCommandType.name(), aDevice.getModemId(), aDevice.getTransactionId(), payLoad.getRwsType().getDesc(), mNumber});
	            if (acLogList.size() > 0) {
	                ...
	                if(rwsType == RWSType.R){
	    	    		...
	    	    		asyncCommandParam.setParamType("MOBILE_NUMBER");	
	    	    		asyncCommandParam.setParamValue(mNumber);
