...
public class ST64 implements java.io.Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6085436636867805544L;
	...
	public void parseLP() {
		...
		int lpcount = (block_count-1)*this.NBR_BLK_INTS_SET1+this.NBR_VALID_INT;
		...
		try{
			if(fst_blk_val_cnt > 0){
				byte[] first_block 
					= new byte[this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT+fst_blk_val_cnt*(this.LEN_EXT_INTERVAL_STATUS+this.LEN_INTERVAL_DATA)];
				first_block 
					= DataFormat.select(
						data,0,this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT+fst_blk_val_cnt*(this.LEN_EXT_INTERVAL_STATUS+this.LEN_INTERVAL_DATA));
				...
				status = getBlkStatus(
									DataFormat.select(
										first_block,this.LEN_BLK_END_TIME,this.LEN_SIMPLE_INTVAL_STAT),fst_blk_val_cnt);
				int statusCnt = 0;
				...
				//마지막 시간 + 유효한 데이터 수
				int preDataSize = this.LEN_BLK_END_TIME+this.LEN_SIMPLE_INTVAL_STAT;
				//데이터 상태 값 + 검침값
				int dataSize 	= this.LEN_EXT_INTERVAL_STATUS+this.LEN_INTERVAL_DATA;
				int interval = 15;
				...
				lp = DataFormat.select(first_block,	preDataSize, fst_blk_val_cnt*dataSize);
			...
			for(int i = 1; i < block_count; i++){
				...
				if(blk[0] == (byte)0xFF){
					...
					status = getBlkStatus(
								DataFormat.select(
									blk,this.LEN_BLK_END_TIME,this.LEN_SIMPLE_INTVAL_STAT),this.NBR_BLK_INTS_SET1);
    ...
    throws Exception
    {
    	...
    	try{
    		...
    		int ofs = 0;//lps.length - (this.LEN_EXT_INTERVAL_STATUS+this.LEN_INTERVAL_DATA);
    		int idx = 0;
    		int timecnt = 0;
    		...
    		for(int i = 0; i < lpcount.intValue(); i++){
    			...
    			for(int k = 0; k < this.NBR_CHANS_SET1;k++){
    				ch[k] = new Double(getCh(lps,ofs,this.LEN_INTERVAL_DATA/this.NBR_CHANS_SET1,k+1));//0));
    				v[k] = new Double(getV(lps,ofs,this.LEN_INTERVAL_DATA/this.NBR_CHANS_SET1,k+1));//0));
...
/*
	public void parseLP() {
		...
		int datalen = data.length;
		int blockcount = data.length/oneblocksize;
		int lpcount = (blockcount-1)*this.NBR_BLK_INTS_SET1+this.NBR_VALID_INT;
		...
		try {
			for(int i = 0; i<blockcount; i++){
				byte[] temp;
				if(i == 0){
					temp = parseBlock(i * oneblocksize, 
										oneblocksize,
										this.NBR_VALID_INT);
				}else{
					temp = parseBlock(i * oneblocksize, 
				...
				System.arraycopy(temp,0,buf,offset,temp.length);
				offset += temp.length;

				if(temp.length == 0){
					totpulsecnt -= this.NBR_BLK_INTS_SET1;
				}
			}
			
			this.lpdata = new byte[offset];
			System.arraycopy(buf,0,lpdata,0,offset);
			this.totpulsecnt += this.emptylpcount;
			logger.debug("lpdata1 : \n"+Util.getHexString(lpdata));
			orderbyLP();
	...
	*/
	...
	public void orderbyLP() throws Exception{
		...
		System.arraycopy(lpdata,0,temp,0,lpdata.length);
		this.lpdata = new byte[temp.length];

		for(int i = 0; i< this.totpulsecnt; i++){
			byte[] block 
				= DataFormat.select(
					temp,i*LP_BLOCK_SIZE,LP_BLOCK_SIZE);

			System.arraycopy(block,0,lpdata,temp.length-(i+1)*LP_BLOCK_SIZE,LP_BLOCK_SIZE);

			if(i == 0){
				this.lpendtime = new byte[6];
				this.lpendtime = DataFormat.select(block,0,6);
			}
			if(i == this.totpulsecnt-1){
				this.lpstarttime = new byte[6];
				this.lpstarttime = DataFormat.select(block,0,6);
			}
		}
	...
	*/
	...
	public byte[] orderby(byte[] notorder) throws Exception{
		...
		byte[] temp = new byte[notorder.length];
		int pulsecnt = notorder.length/LP_BLOCK_SIZE;
		...
		for(int i = 0; i< pulsecnt ; i++){
			...
			System.arraycopy(block,0,temp,notorder.length-(i+1)*LP_BLOCK_SIZE,LP_BLOCK_SIZE);
		}
	...
	*/
	...
	/*
	public byte[] parseBlock(int offset, int len, int validcnt) 
					throws Exception {
		
		byte[] lpblock = new byte[validcnt*this.LP_BLOCK_SIZE];

		byte[] temp = DataFormat.select(data,offset,len);
		boolean validlp;

		String endtime = new String();
		
		if(Util.getNHexString(DataFormat.select(temp,0,6)).equals("ffffffffffff")){
			logger.warn("Not Valid Block ");
			return new byte[0];
		}
		try{
			if(validcnt != this.NBR_BLK_INTS_SET1){
				endtime = getQuaterYymmddhhmm(getYymmddhhmm(temp,0,LEN_BLK_END_TIME));
			}else{
				endtime = Util.getQuaterYymmddhhmm(getYymmddhhmm(temp,0,LEN_BLK_END_TIME));
			}
		}catch(Exception e){
			logger.warn("Not Valid Block : "+e.getMessage());
			return new byte[0];
		}

		String starttime = Util.addMinYymmdd(endtime,-((validcnt-1)*INT_TIME_SET1));

		int ofs = LEN_BLK_END_TIME;
		int blkstat = DataFormat.hex2dec(
						DataFormat.LSB2MSB(
							DataFormat.select(temp,ofs,LEN_SIMPLE_INTVAL_STAT))); 
		ofs += LEN_SIMPLE_INTVAL_STAT;
		int pos = 0;

		int emptycount = 0;
		if(blockendtime != null && validcnt > 0){
			emptycount = getEmptyCount(endtime,blockendtime);
			this.emptylpcount += emptycount;
			lpblock = new byte[(validcnt+emptycount)*this.LP_BLOCK_SIZE];
		}

		for(int i = 0; i < validcnt; i++){
						
			byte[] time = FEPUtil.strDate2Hex(Util.addMinYymmdd(starttime,(i*INT_TIME_SET1)),6);
			System.arraycopy(time,0,lpblock,pos,time.length);
			pos+= time.length;//6

			byte[] stat = DataFormat.select(temp,ofs,LEN_EXT_INTERVAL_STATUS);
			ofs += LEN_EXT_INTERVAL_STATUS;
			...
			if(validlp){
				for(int j = 0; j < NBR_CHANS_SET1;j++){
					byte[] ch = parseCh(temp,ofs,LEN_INTERVAL_DATA/NBR_CHANS_SET1);
					ofs += LEN_INTERVAL_DATA/NBR_CHANS_SET1;
					System.arraycopy(ch,0,lpblock,pos,ch.length);
					pos += 4;
				}
			}else {
				for(int j = 0; j < NBR_CHANS_SET1;j++){
					byte[] ch = new byte[4];//setting zero(0)
					ofs += LEN_INTERVAL_DATA/NBR_CHANS_SET1;
					System.arraycopy(ch,0,lpblock,pos,ch.length);
					pos += 4;
				}
			...
			System.arraycopy(stat,0,lpblock,pos,LEN_EXT_INTERVAL_STATUS);			
			pos += 2;

		}
			
		for(int i = 0;i < emptycount; i++){
			byte[] time = FEPUtil.strDate2Hex(Util.addMinYymmdd(endtime,((i+1)*INT_TIME_SET1)),6);
			System.arraycopy(time,0,lpblock,pos,time.length);
			pos+= time.length;//6
			for(int j = 0; j < NBR_CHANS_SET1;j++){
				byte[] ch = new byte[4];
			...
			System.arraycopy(stat,0,lpblock,pos,stat.length);			
			pos += stat.length;	
		}
		
		if(validcnt > 0){
			this.blockendtime = starttime;
		}
	...
	*/
    ...
    private double getCh(byte[] b, int start, int len,int ch)
        ...
        return (val*(ke*Math.pow(10,this.meterConstantScalel))*(st062.getDIVISORst64(ch)/st062.getSCALARst64(ch)));
    ...
    private double getV(byte[] b, int start, int len,int ch)
        ...
        return (val*(ke*Math.pow(10,this.meterConstantScalel))*(st062.getDIVISORst64(ch)/st062.getSCALARst64(ch)))*60/this.INT_TIME_SET1;
    }
	...
	private String getBlkStatus(byte[] b, int blkcnt){
		...
		for(int i = 0; i < b.length; i++){
			...
			StringBuffer temp = new StringBuffer(Util.frontAppendNStr('0',Integer.toBinaryString(val),8));
			str += temp.reverse();
		}
