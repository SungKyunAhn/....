...
import com.aimir.fep.bypass.dlms.enums.AutoConnectMode;
...
public class DLMSAutoConnect extends DLMSObject implements DLMSBase
{
    private AutoConnectMode Mode;
    private List<AbstractMap.SimpleEntry<DLMSDateTime, DLMSDateTime>> m_CallingWindow;
    ...
    */
    public DLMSAutoConnect(String ln)
    {
        this(ln, (short) 0);
    }
    ...
    */
    public DLMSAutoConnect(String ln, int sn)
    {
        super(ObjectType.AUTO_CONNECT, ln, sn);
        m_CallingWindow = new java.util.ArrayList<AbstractMap.SimpleEntry<DLMSDateTime, DLMSDateTime>>();
    }
    ...
    public final List<AbstractMap.SimpleEntry<DLMSDateTime, DLMSDateTime>> getCallingWindow()
    {
        return m_CallingWindow;
    }
    public final void setCallingWindow(List<AbstractMap.SimpleEntry<DLMSDateTime, DLMSDateTime>> value)
    {
    ...
    public Object getValue(int index, int selector, Object parameters)
    {
        ...
        if (index == 5)
        {
            ...
            if (cnt != 0)
            {                
                for (AbstractMap.SimpleEntry<DLMSDateTime, DLMSDateTime> it : getCallingWindow())
                {
                    data.write((byte)DataType.STRUCTURE.getValue());
                    data.write((byte)2); //Count
                    try 
                    {
                        Common.setData(data, DataType.OCTET_STRING, it.getKey()); //start_time
                        Common.setData(data, DataType.OCTET_STRING, it.getValue()); //end_time
                    }
                    catch (Exception ex) 
                    {
                        Logger.getLogger(DLMSAutoConnect.class.getName()).log(Level.SEVERE, null, ex);
        ...
        if (index == 6)
        {
            ...
            else
            {
                ...
                for (String it : getDestinations())
                {
                    try 
                    {
                        Common.setData(data, DataType.OCTET_STRING, Common.getBytes(it)); //destination
                    }
                    catch (Exception ex) 
                    {
                        Logger.getLogger(DLMSAutoConnect.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void setValue(int index, Object value)
    {
        ...
        else if (index == 5)
        {
            ...
            if (value != null)
            {
                ...
                {
                    DLMSDateTime start = (DLMSDateTime)DLMSClient.changeType((byte[])Array.get(item, 0), DataType.DATETIME);
                    DLMSDateTime end = (DLMSDateTime)DLMSClient.changeType((byte[])Array.get(item, 1), DataType.DATETIME);                    
        ...
        else if (index == 6)
        {
            ...
            if (value != null)
            {
                ...
                for (Object item : (Object[])value)
                {
                    String it = DLMSClient.changeType((byte[]) item, DataType.STRING).toString();
                    items.add(it);
                }
