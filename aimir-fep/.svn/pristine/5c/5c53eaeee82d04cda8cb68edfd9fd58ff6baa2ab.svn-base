...
public class ModbusEMnVRockwell extends MeterDataParser implements java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private static Logger log = LoggerFactory.getLogger(ModbusEMnVRockwell.class);
	...
	LinkedHashMap<MODBUS_ROCKWELL_CODE, Map<String, Object>> result = new LinkedHashMap<MODBUS_ROCKWELL_CODE, Map<String, Object>>();
	...
	public void parse(byte[] data) throws Exception {
		...
		log.info("    ");
		log.info("    ");
		log.info("################ ROCKWEL Inverter 로그확인 시작 ({}) #########################", parsingTime);
		log.info("ModbusEMnV-ROCKWEL parse:[{}] [{}]", meteringDataType != null ? meteringDataType.name() : "", Hex.decode(data));
		...
		log.info("총 MODBUS_ROCKWELL_CODE 갯수 ==> {}", dataCount);

		// 가짜 LP타임 저장
		parsingTime = new ModebusInverterCommonTable().resetLpTime(cal);
		setMCodeResult(MODBUS_ROCKWELL_CODE.DATE, parsingTime);
		...
		for (int i = 0; i < dataCount; i++) {
			...
			MODBUS_ROCKWELL_CODE mCode = MODBUS_ROCKWELL_CODE.getItem(Hex.decode(address));
			...
			case OUTPUT_POWER:
				value = Hex.decode(mData);
				break;
			case ACCUMULATED_MWH:
				value = Hex.decode(mData);
				break;
			case ACCUMULATED_DRIVIN_TIME:
				value = Hex.decode(mData);
				break;
			case TORQUE_CURRENT:
				value = Hex.decode(mData);
			...
			log.info("[{}][PROTOCOL][METERING_DATA] MODBUS_ROCKWELL_CODE[{}] ==> {}={}, HEX=[{}]", new Object[] { i, Hex.decode(address), mCode.name(), value, Hex.decode(mData) });
		}

		log.debug("### MODBUS_ROCKWELL_CODE TOTAL ==> {}", result.toString());
		log.info("### Number_of_MODBUS={} Last POS={}", new Object[] { dataCount, pos });

		setInverterInfo();
	}

	private void setMCodeResult(MODBUS_ROCKWELL_CODE mCode, Object value) {
	...
	public void setLPData() {
		try {
			...
			// 출력 주파수
			mCodeMap = result.get(MODBUS_ROCKWELL_CODE.OUTPUT_FREQUENCY);
			if (mCodeMap != null) {
			...
			// 출력 전류
			mCodeMap = result.get(MODBUS_ROCKWELL_CODE.OUTPUT_CURRENT);
			if (mCodeMap != null) {
			...
			// 출력 전압
			mCodeMap = result.get(MODBUS_ROCKWELL_CODE.OUTPUT_VOLTAGE);
			if (mCodeMap != null) {
			...
			lpEl = new LPData((result.get(MODBUS_ROCKWELL_CODE.DATE)).get("VALUE").toString(), lp, lpValue);
