...
public class DLMSModemConfiguration extends DLMSObject implements DLMSBase
{
    ...
    */
    public DLMSModemConfiguration()
    {
        ...
        m_InitialisationStrings = new DLMSModemInitialisation[0];
    ...
    */
    public DLMSModemConfiguration(String ln)
    {
    ...
    */
    public DLMSModemConfiguration(String ln, int sn)
    {
        super(ObjectType.MODEM_CONFIGURATION, ln, 0);
        m_InitialisationStrings = new DLMSModemInitialisation[0];
        m_CommunicationSpeed = BaudRate.BAUDRATE_300;
    ...
    public final void setInitialisationStrings(DLMSModemInitialisation[] value)
    {
    ...
    public Object getValue(int index, int selector, Object parameters)
    {
        ...
        if (index == 3)
        {
            ...
            if (cnt != 0)
            {
                try 
                {                
                    for (DLMSModemInitialisation it : m_InitialisationStrings)
                    {
                        data.write(DataType.STRUCTURE.getValue());
                        data.write(3); //Count                        
                        Common.setData(data, DataType.OCTET_STRING, Common.getBytes(it.getRequest()));
                        Common.setData(data, DataType.OCTET_STRING, Common.getBytes(it.getResponse()));
                        Common.setData(data, DataType.UINT16, it.getDelay());                    
                    }
                } 
                catch (Exception ex) 
                {                    
                    Logger.getLogger(DLMSModemConfiguration.class.getName()).log(Level.SEVERE, null, ex);
        ...
        if (index == 4)
        {
            ...
            if (cnt != 0)
            {
                ...
                {                
                    ...
                    {
                        Common.setData(data, DataType.OCTET_STRING, Common.getBytes(it));
                    }
                }
                catch (Exception ex) 
                {
    ...
    public void setValue(int index, Object value)
    {
        ...
        else if (index == 3)
        {
            ...
            if (value != null)
            {                
                List<DLMSModemInitialisation> items = new ArrayList<DLMSModemInitialisation>();
                for(Object it : (Object[])value)
                {
                    DLMSModemInitialisation item = new DLMSModemInitialisation();                                        
                    item.setRequest(DLMSClient.changeType((byte[])Array.get(it, 0), DataType.STRING).toString());
                    item.setResponse(DLMSClient.changeType((byte[])Array.get(it, 1), DataType.STRING).toString());
                ...
                m_InitialisationStrings = items.toArray(new DLMSModemInitialisation[items.size()]);
            }                                 
        }
        else if (index == 4)
        {
            ...
            if (value != null)
            {
                ...
                for (Object it : (Object[])value)
                {
                    items.add(DLMSClient.changeType((byte[])it, DataType.STRING).toString());
                }
