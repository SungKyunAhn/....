 ...
 *
 */
public class EMnVLPIntervalAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVLPIntervalAction.class);
	private EMnVCommandSubFrameType commandType;
	private RWSType rwsType;
	private ActionCommandType actionCommandType = ActionCommandType.LP_INTERVAL;

	public EMnVLPIntervalAction(EMnVCommandSubFrameType commandType) {
	...
	public void execute(EMnVGeneralFramePayLoad gFramePayload, IoSession session, EMnVGeneralDataFrame generalDataFrame) throws Exception {
            ...
            if (acllist.size() > 0) {
               ...
               String lpInterval = null;
                for(AsyncCommandParam param : acplist){
                	if(param.getParamType().equals("lp_interval")){
                		lpInterval = param.getParamValue();
                	}
                }
                ...
                if(rwsType == RWSType.W){
                    payLoad.setInterval(new byte[]{DataUtil.getByteToInt(lpInterval)});
                    HashMap<String, Object> params = new HashMap<String, Object>();
                    params.put("w_lp_interval", lpInterval);
                    aDevice.setParams(params);
                }
                generalDataFrame.setGeneralFramePayLoad(payLoad);
                
                logger.info("");
                logger.info("#### [{}][COMMAND_REQUEST][TRAN_ID={}][remote={}] RWS_Type={}, lp_interval={}"
                		, new Object[]{actionCommandType.name(), aDevice.getTransactionId()
                				, session.getRemoteAddress(), rwsType, lpInterval});
		...
		case COMMAND_RESPONSE:
			...
			case SUCCESS:
				logger.info("[{}][COMMAND_RESPONSE][{}][TRNX_ID={}][{}-{}] Success !!!"
						, new Object[]{actionCommandType.name(), aDevice.getModemId(), aDevice.getTransactionId(), payLoad.getRwsType().getDesc()
								, rwsType == RWSType.W ? aDevice.getParams().get("w_lp_interval") : DataUtil.getIntToBytes((payLoad.getInterval()))});
	            if (acLogList.size() > 0) {
	                ...
	                if(rwsType == RWSType.R){
	    	    		...
	    	    		asyncCommandParam.setParamType("LP_INTERVAL");	
			...
			case NOT_SUPPORTED_COMMAND:
				logger.error("[LP_INTERVAL][COMMAND_RESPONSE][{}][TRNX_ID={}] Fail !!! - NOT_SUPPORTED_COMMAND ", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case NOT_ACCESS_USER:
				logger.error("[LP_INTERVAL][COMMAND_RESPONSE][{}][TRNX_ID={}] Fail !!! - NOT_ACCESS_USER", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case FAIL:
				logger.error("[LP_INTERVAL][COMMAND_RESPONSE][{}][TRNX_ID={}] Fail !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
	                for (int i = 0; i < acLogList.size(); i++) {
	                	AsyncCommandLog acl = acLogList.get(i);
