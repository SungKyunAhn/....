 ...
 *
 */
public class EMnVHWResetIntervalAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVHWResetIntervalAction.class);
	private EMnVCommandSubFrameType commandType;
	private RWSType rwsType;
	private ActionCommandType actionCommandType = ActionCommandType.HW_RESET_INTERVAL;

	public EMnVHWResetIntervalAction(EMnVCommandSubFrameType commandType) {
	...
	public void execute(EMnVGeneralFramePayLoad gFramePayload, IoSession session, EMnVGeneralDataFrame generalDataFrame) throws Exception {
		...
		case COMMAND_REQUEST:     
            ...
            if (acllist.size() > 0) {
               ...
               String hwResetInterval = null;
                for(AsyncCommandParam param : acplist){
                	if(param.getParamType().equals("hw_reset_interval")){
                		hwResetInterval = param.getParamValue();
                	}
                }
                
                if(hwResetInterval == null){
                	rwsType = RWSType.R;
                }else{
                	rwsType = RWSType.W;
                }
                ...
                if(rwsType == RWSType.W){
                    payLoad.setInterval(new byte[]{DataUtil.getByteToInt(hwResetInterval)});
                    HashMap<String, Object> params = new HashMap<String, Object>();
                    params.put("w_hw_reset_interval", hwResetInterval);
                    aDevice.setParams(params);
                }
                generalDataFrame.setGeneralFramePayLoad(payLoad);
                
                logger.info("");
                logger.info("#### [{}][COMMAND_REQUEST][TRAN_ID={}][remote={}] RWS_Type={}, hw_reset_interval={}"
                		, new Object[]{actionCommandType.name(), aDevice.getTransactionId()
                				, session.getRemoteAddress(), rwsType, hwResetInterval});
		...
		case COMMAND_RESPONSE:
			...
			case SUCCESS:
				logger.info("[{}][COMMAND_RESPONSE][{}][TRNX_ID={}][{}-{}] Success !!!"
						, new Object[]{actionCommandType.name(), aDevice.getModemId(), aDevice.getTransactionId(), payLoad.getRwsType().getDesc()
								, rwsType == RWSType.W ? aDevice.getParams().get("w_hw_reset_interval") : DataUtil.getIntToBytes((payLoad.getInterval()))});
	            if (acLogList.size() > 0) {
	                ...
	                if(rwsType == RWSType.R){
	    	    		...
	    	    		asyncCommandParam.setParamType("HW_RESET_INTERVAL");	
	    	    		asyncCommandParam.setParamValue(String.valueOf(DataUtil.getIntToBytes(payLoad.getInterval())));
			...
			case NOT_SUPPORTED_COMMAND:
				logger.error("[HW_RESET_INTERVAL][COMMAND_RESPONSE][{}][TRNX_ID={}] Fail !!! - NOT_SUPPORTED_COMMAND ", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case NOT_ACCESS_USER:
				logger.error("[HW_RESET_INTERVAL][COMMAND_RESPONSE][{}][TRNX_ID={}] Fail !!! - NOT_ACCESS_USER", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case FAIL:
				logger.error("[HW_RESET_INTERVAL][COMMAND_RESPONSE][{}][TRNX_ID={}] Fail !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
	                for (int i = 0; i < acLogList.size(); i++) {
	                	AsyncCommandLog acl = acLogList.get(i);
