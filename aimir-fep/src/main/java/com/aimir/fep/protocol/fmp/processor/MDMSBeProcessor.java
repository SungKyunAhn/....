 ...
 */
@Component
public class MDMSBeProcessor extends Processor {
	private static Log logger = LogFactory.getLog(MDMSBeProcessor.class);
	
	@Autowired
	OperationUtilAction operationUtilAction;
	
	private final String HEAD_NAME = "IOT_CI_BE";
	...
	private final String parentPath = FMPProperty.getProperty("mdms.file.fepd.path");
	...
	public MDMSBeProcessor() throws Exception {
	 ...
	 * @param resource
	 * @throws Exception
	 * .tmp 파일이 있는경우 데이터가 덮어진다. 
	 * 없는 경우 ->tmp 파일을 만들어 데이터를 저장한다.
	 * .dat 파일의 경우 스케쥴 주기에 따라 만들어진다. by ask
	 */
	public synchronized void writeFile(Resource resource) throws Exception {
		...
		try {
			...
			logger.debug("["+seq+"] Write MDMS-BE log Data");
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
