...
public class EMnVMeterTimeSyncAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVMeterTimeSyncAction.class);
	private EMnVCommandSubFrameType commandType;

	public EMnVMeterTimeSyncAction(EMnVCommandSubFrameType commandType) {
	...
	public void execute(EMnVGeneralFramePayLoad gFramePayload, IoSession session, EMnVGeneralDataFrame generalDataFrame) throws Exception {
		...
		case COMMAND_REQUEST:
            ...
            logger.debug("[{}][COMMAND_REQUEST][TRAN_ID={}] - DEVICE={}, COMMAND={}, STATE={}, ASYNC_SIZE={}"
            		, new Object[]{ActionCommandType.METER_TIMESYNC.name(), aDevice.getTransactionId(), aDevice.getModemId(), aDevice.getCommand(), TR_STATE.Waiting.getCode(), acllist.size ()});
            if (acllist.size() > 0) {
            	/*
                ...
                payLoad.setCommandType(ActionCommandType.METER_TIMESYNC);
                ...
                logger.info("");
                logger.info("#### [METER_TIMESYNC][COMMAND_REQUEST][TRAN_ID={}][remote={}] hdlc_address={}"
                		, new Object[]{aDevice.getTransactionId(), session.getRemoteAddress(), hdlcAddress});
		...
		case COMMAND_RESPONSE:
			...
			case SUCCESS:
				logger.info("[METER_TIMESYNC][COMMAND_RESPONSE][{}][TRNX_ID={}] Meter Timesync Success !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
			...
			case NOT_SUPPORTED_COMMAND:
				logger.error("[METER_TIMESYNC][COMMAND_RESPONSE][{}][TRNX_ID={}] Meter Timesync Fail !!! - NOT_SUPPORTED_COMMAND ", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case NOT_ACCESS_USER:
				logger.error("[METER_TIMESYNC][COMMAND_RESPONSE][{}][TRNX_ID={}] Meter Timesync Fail !!! - NOT_ACCESS_USER", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case FAIL:
				logger.error("[METER_TIMESYNC][COMMAND_RESPONSE][{}][TRNX_ID={}] Meter Timesync Fail !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
	                for (int i = 0; i < acLogList.size(); i++) {
	                	AsyncCommandLog acl = acLogList.get(i);
