 ...
 *
 */
public class EMnVInverterInfoAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVInverterInfoAction.class);
	private EMnVCommandSubFrameType commandType;
	private RWSType rwsType;
	private ActionCommandType actionCommandType = ActionCommandType.INVERTER_INFO;

	public EMnVInverterInfoAction(EMnVCommandSubFrameType commandType) {
	...
	public void execute(EMnVGeneralFramePayLoad gFramePayload, IoSession session, EMnVGeneralDataFrame generalDataFrame) throws Exception {
		...
		case COMMAND_REQUEST:
            ...
            logger.debug("[INVERTER_INFO][COMMAND_REQUEST][TRAN_ID={}] - DEVICE={}, COMMAND={}, STATE={}, ASYNC_SIZE={}"
            ...
            if (acllist.size() > 0) {
                ...
                logger.info("");
                logger.info("#### [INVERTER_INFO][COMMAND_REQUEST][TRAN_ID={}][remote={}]"
                		, new Object[]{aDevice.getTransactionId(), session.getRemoteAddress()});
		...
		case COMMAND_RESPONSE:
			...
			case SUCCESS:
				logger.info("[INVERTER_INFO][COMMAND_RESPONSE][{}][TRNX_ID={}] INVERTER_INFO Success !!!"
						, new Object[]{aDevice.getModemId(), aDevice.getTransactionId()});
	            ...
	            if (acLogList.size() > 0) {
    	    		...
    	    		asyncCommandParam.setParamType("MAX_PORT_SIZE");	
    	    		asyncCommandParam.setParamValue(String.valueOf(payLoad.getMaxPortSize()));
    	    		asyncCommandParam.setTrId(aDevice.getTransactionId());

                    AsyncCommandParamDao acpd = DataUtil.getBean(AsyncCommandParamDao.class);
    	    		acpd.add(asyncCommandParam);
    	    		
    	    		logger.info("[INVERTER_INFO] ==> MAX_PORT_SIZE={}", payLoad.getMaxPortSize());
    	    		
    	    		int num = 2;
                	for(HashMap<String, Object> map : payLoad.getInverterInfoList()){
        				asyncCommandParam = new AsyncCommandParam();
        	    		asyncCommandParam.setMcuId(aDevice.getModemId());
        	    		asyncCommandParam.setNum(num);
        	    		asyncCommandParam.setParamType("INVERTER_INFO");	
        	    		asyncCommandParam.setParamValue(map.toString());
        	    		asyncCommandParam.setTrId(aDevice.getTransactionId());

        	    		acpd.add(asyncCommandParam);
        	    		num++;
        	    		
        	    		logger.info("[INVERTER_INFO] ==> {}", map.toString());
                	}
			...
			case NOT_SUPPORTED_COMMAND:
				logger.error("[INVERTER_INFO][COMMAND_RESPONSE][{}][TRNX_ID={}] Fail !!! - NOT_SUPPORTED_COMMAND ", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case NOT_ACCESS_USER:
				logger.error("[INVERTER_INFO][COMMAND_RESPONSE][{}][TRNX_ID={}] Fail !!! - NOT_ACCESS_USER", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case FAIL:
				logger.error("[INVERTER_INFO][COMMAND_RESPONSE][{}][TRNX_ID={}] Fail !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
	                for (int i = 0; i < acLogList.size(); i++) {
	                	AsyncCommandLog acl = acLogList.get(i);
