...
public class A1RLQ_Handler extends MeterProtocolHandler {
	...
	public CommandData execute(MRPClientProtocolHandler handler,
        ...
        log.debug("==============A1RL_Handler start cmd:"+cmd+"================");
        if(cmd.equals("104.6.0")||cmd.equals("104.13.0"))
        {
            ...
            if(smiValue != null && smiValue.length >= 2){
                ...
                ba.append(new byte[]{0x00,0x00,0x00,0x00,0x00});//FW VER
                ba.append(new byte[]{0x00});
                ba.append(mcuId);
                ba.append(new byte[]{(byte)0x8C});//A1RL_Handler
            ...
            {      
                ...
                md.setMdType(new BYTE(Integer.parseInt(CommonConstants.getModemTypeCode(ModemType.MMIU))));//TODO 
    ...
    public ByteArray lpRead(IoSession session, int nOffset, int nCount) throws MRPException
    {
        ...
        BYTE control = new BYTE(A1RLQ_DataConstants.CB_CLASSREAD);
        BYTE function = null;
        BYTE pad = new BYTE(A1RLQ_DataConstants.PAD_NULL);
        
    	OCTET length = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00});
    	OCTET send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_12});

        byte[] temp = null;
        ByteArray ba = new ByteArray();     
        
        A1RLQ_RequestDataFrame frame = null;
        A1RLQ_ReceiveDataFrame buf = null;
        try{
        	//current value
        	frame = new A1RLQ_RequestDataFrame(new BYTE(), control, function, pad, length, send_data, 0,0, null);
            buf = read(session,frame);        
            temp = buf.encode();
            
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_ETKWH1, A1RLQ_DataConstants.LEN_ETKWH1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_ETKWH2, A1RLQ_DataConstants.LEN_ETKWH2, temp, 0, 0));
            
            //LP
            send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_14});
        	frame = new A1RLQ_RequestDataFrame(new BYTE(), control, function, pad, length, send_data, 0,0, null);
            buf = read(session,frame);        
            temp = buf.encode();
                  
            int channelCnt = temp[A1RLQ_DataConstants.OFS_CHANS];
            int dayCnt = temp[A1RLQ_DataConstants.OFS_LPMEM];
            int dasize  = DataUtil.getIntTo2Byte(DataUtil.select(temp, A1RLQ_DataConstants.OFS_DASIZE, A1RLQ_DataConstants.LEN_DASIZE));
        	...
        	send_data = new OCTET(new byte[]{(byte) 0x00, (byte) (nOffset & 0xff), A1RLQ_DataConstants.CLASS_18});
	...
	public ByteArray billRead(IoSession session) throws MRPException {
    	...
    	OCTET length = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00});
    	OCTET send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_9});
        ...
        try{
            ...
            buf = read(session,frame);        
            temp = buf.encode();
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CUMDR, A1RLQ_DataConstants.LEN_CUMDR,
            								temp, A1RLQ_DataConstants.OFS_DATATR, A1RLQ_DataConstants.LEN_DATATR));
            
            send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_12});
            frame = new A1RLQ_RequestDataFrame(new BYTE(), control, function, pad, length, send_data, 0,0, null);
            buf = read(session,frame);        
            temp = buf.encode();
            ...
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_AKWH1, A1RLQ_DataConstants.LEN_AKWH1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_BKWH1, A1RLQ_DataConstants.LEN_BKWH1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CKWH1, A1RLQ_DataConstants.LEN_CKWH1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_AKWH2, A1RLQ_DataConstants.LEN_AKWH2, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_BKWH2, A1RLQ_DataConstants.LEN_BKWH2, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CKWH2, A1RLQ_DataConstants.LEN_CKWH2, temp, 0, 0));
            
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_AKW1, A1RLQ_DataConstants.LEN_AKW1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_ATD1, A1RLQ_DataConstants.LEN_ATD1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_BKW1, A1RLQ_DataConstants.LEN_BKW1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_BTD1, A1RLQ_DataConstants.LEN_BTD1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CKW1, A1RLQ_DataConstants.LEN_CKW1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CTD1, A1RLQ_DataConstants.LEN_CTD1, temp, 0, 0));
            
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_AKW2, A1RLQ_DataConstants.LEN_AKW2, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_ATD2, A1RLQ_DataConstants.LEN_ATD2, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_BKW2, A1RLQ_DataConstants.LEN_BKW2, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_BTD2, A1RLQ_DataConstants.LEN_BTD2, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CKW2, A1RLQ_DataConstants.LEN_CKW2, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CTD2, A1RLQ_DataConstants.LEN_CTD2, temp, 0, 0));
            
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_AKWCUM1, A1RLQ_DataConstants.LEN_AKWCUM1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_BKWCUM1, A1RLQ_DataConstants.LEN_BKWCUM1, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CKWCUM1, A1RLQ_DataConstants.LEN_CKWCUM1, temp, 0, 0));
            
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_AKWCUM2, A1RLQ_DataConstants.LEN_AKWCUM2, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_BKWCUM2, A1RLQ_DataConstants.LEN_BKWCUM2, temp, 0, 0));
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CKWCUM2, A1RLQ_DataConstants.LEN_CKWCUM2, temp, 0, 0));
			
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_EAVGPF, A1RLQ_DataConstants.LEN_EAVGPF, temp, 0, 0));
    ...
    public ByteArray configureRead(IoSession session) throws MRPException
    {
    	...
    	OCTET send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x05, A1RLQ_DataConstants.CLASS_10});
        ...
        try{
            ...
            ba.append(DataUtil.arrayAppend(temp, 0, A1RLQ_DataConstants.LEN_MTRSN, temp, 0, 0));
            //
            length = new OCTET(new byte[]{(byte) 0x00, (byte) A1RLQ_DataConstants.LEN_CLASS9});
            send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_9});
            frame = new A1RLQ_RequestDataFrame(new BYTE(), control, function, pad, length, send_data, 0,0, null);
            buf = read(session,frame);        
            temp = buf.encode();
             
     		byte[] stat = new byte[5];
     		stat[0] = temp[A1RLQ_DataConstants.OFS_SYSWARN];
    		System.arraycopy(temp,A1RLQ_DataConstants.OFS_SYSERR,stat,1,A1RLQ_DataConstants.LEN_SYSERR);
    		stat[4] = temp[A1RLQ_DataConstants.OFS_SYSSTAT];
    		
    		byte[] datetime = new byte[A1RLQ_DataConstants.LEN_TD]; 
    		System.arraycopy(temp, A1RLQ_DataConstants.OFS_TD, datetime, 0, A1RLQ_DataConstants.LEN_TD);
    		     
    	//	ba.append(b);
    	//	ba.append((byte)0x0F);//resolution

    		send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_6});
            frame = new A1RLQ_RequestDataFrame(new BYTE(), control, function, pad, length, send_data, 0,0, null);
            buf = read(session,frame);        
            temp = buf.encode();
            
            ba.append(parseXUME(temp[A1RLQ_DataConstants.OFS_XUME]));//meter element

    		length = new OCTET(new byte[]{(byte) 0x00, (byte) A1RLQ_DataConstants.LEN_CLASS0});    		
            send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_0});
            frame = new A1RLQ_RequestDataFrame(new BYTE(), control, function, pad, length, send_data, 0,0, null);
            buf = read(session,frame);        
            temp = buf.encode();
                        
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_UKE, A1RLQ_DataConstants.LEN_UKE, temp, 0, 0));
            ba.append(stat);
            ba.append(new byte[]{(byte)0x00});
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_CTRATIO, A1RLQ_DataConstants.LEN_CTRATIO, temp, A1RLQ_DataConstants.OFS_VTRATIO, A1RLQ_DataConstants.LEN_VTRATIO));
            ba.append(new byte[]{(byte)0x00});
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_DPLOCD, A1RLQ_DataConstants.LEN_DPLOCD, temp, 0,0));
            ba.append(new byte[]{(byte)0x00});
            ba.append(DataUtil.arrayAppend(temp, A1RLQ_DataConstants.OFS_DPLOCE, A1RLQ_DataConstants.LEN_DPLOCE, temp, 0,0));
	...
	private byte parseXUME(byte data) throws Exception {
	...
	public ByteArray eventlogRead(IoSession session) throws MRPException {
    	...
    	OCTET length = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00});
    	OCTET send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_16});
	...
	public ByteArray instRead(IoSession session) throws MRPException {
        ...
        BYTE control = new BYTE(A1RLQ_DataConstants.CB_INSTRUMENT);
        BYTE function = null;
	...
	public ByteArray pqRead(IoSession session) throws MRPException {
    	...
    	OCTET length = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00});
    	OCTET send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_50});
        ...
        try{
            ...
            //sag
            send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_52});
        	frame = new A1RLQ_RequestDataFrame(new BYTE(), control, function, pad, length, send_data, 0,0, null);
            buf = read(session,frame);        
            temp = buf.encode();
            ...
            //sag event
            send_data = new OCTET(new byte[]{(byte) 0x00, (byte) 0x00, A1RLQ_DataConstants.CLASS_53});
        	frame = new A1RLQ_RequestDataFrame(new BYTE(), control, function, pad, length, send_data, 0,0, null);
        	buf = read(session,frame);
        	temp = buf.encode();
        	
        	int count =0;
        	for(int i=0; i< A1RLQ_DataConstants.MAX_BLOCK_COUNT; i++){
        		if(temp[i*A1RLQ_DataConstants.ONE_BLOCK_SIZE]==0x00)
        			break;
        		else count++;
        	}
        	ba.append(new byte[]{(byte)count});
        	ba.append(DataUtil.arrayAppend(temp, 0,count*A1RLQ_DataConstants.ONE_BLOCK_SIZE, temp, 0, 0));
	...
	/**
	 * Command Who Are You? <p>
	 * 
	 * SEQUENCE ex) 
	 * ------------------------------------------------------------
	 * |STX(1)|CB(1)|FUNC(1)|PAD(1)|LEN(1)|DATA(1)|CRCH(1)|CRCL(1)|
	 * ------------------------------------------------------------
	 * | 0x02 |0x18 | 0x06  | 0x00 | 0x01 | 0x00  |  ??   |  ??   |
	 * ------------------------------------------------------------
	 * 
    ...
    {
        BYTE control = new BYTE(A1RLQ_DataConstants.CB_WHOAREYOU);
        BYTE function = new BYTE(A1RLQ_DataConstants.FUNC_WHOAREYOU);
        BYTE pad = new BYTE(A1RLQ_DataConstants.PAD_TIMEOUT);
        OCTET length = new OCTET(new byte[]{(byte) 0x01});
        OCTET send_Data = new OCTET(new byte[]{(byte) 0x00});

        A1RLQ_RequestDataFrame frame 
            	= new A1RLQ_RequestDataFrame(new BYTE(), control, function, pad, length, send_Data, 0,0, null);
        ...
        try
        {
            ...
            /* Receive Wrong Data Response. */
     		if ((message.length != 15) || (message[0] != A1RLQ_DataConstants.STX))
	...
	/** 
	 * Make Password Command.<p>
	 * 
	 * @see aimir.fep.connection.MeterConnector#passwdCheck(StringBuffer key)
	 * 
	 * SEQUENCE ex) 
	 * ------------------------------------------------------------
	 * |STX(1)|CB(1)|FUNC(1)|PAD(1)|LEN(1)|DATA(4)|CRCH(1)|CRCL(1)|
	 * ------------------------------------------------------------
	 * | 0x02 | 0x16| 0x01  | 0x00 | 0x01 | ????  |  ??   |  ??   |
	 * ------------------------------------------------------------
	 */
    protected boolean passwdCheck(IoSession session, byte[] ekey) throws MRPException
    {
        BYTE control = new BYTE(A1RLQ_DataConstants.CB_WHOAREYOU);
        BYTE function = new BYTE(A1RLQ_DataConstants.FUNC_PASSWDCHECK);
        BYTE pad = new BYTE(A1RLQ_DataConstants.PAD_NULL);
        ...
        try
        {
        	...
        	if(message!=null && message.length ==6 && message[0] == A1RLQ_DataConstants.STX && message[2]==A1RLQ_DataConstants.ACK)
        		return true;
        	else
    ...
    {
    	 BYTE control = new BYTE(A1RLQ_DataConstants.CB_TERMINATE);
    	 
         A1RLQ_RequestDataFrame frame 
             = new A1RLQ_RequestDataFrame(new BYTE(), control, 0,0, null);
         try
         {
	         IoBuffer buf = frame.getIoBuffer();
	         log.debug("send passwdCheck=>"+buf.getHexDump());
    ...
    protected A1RLQ_ReceiveDataFrame read(IoSession session, A1RLQ_RequestDataFrame frame, int a) throws MRPException
    {
        int sequence = 0;
        try
        {
            IoBuffer buf = null;
            A1RLQ_ReceiveDataFrame rcvFrame = null;
        	...
        	while(cnt<retryCnt){
	        	...
	        	if(message != null && message.length > 0){
	               	if(message[0] != A1RLQ_DataConstants.STX || message[2] != A1RLQ_DataConstants.ACK)
	               	{
	               		...
	               		rcvFrame = new A1RLQ_ReceiveDataFrame();
    ...
    protected A1RLQ_ReceiveDataFrame read(IoSession session, A1RLQ_RequestDataFrame frame) throws MRPException
    {
        int sequence = 0;
        try
        {
            IoBuffer buf = null;
            A1RLQ_ReceiveDataFrame rcvFrame = new A1RLQ_ReceiveDataFrame();
        	...
        	while(isContinue){
	        	...
	        	if(message != null && message.length > 0){
	               	if(message[0] != A1RLQ_DataConstants.STX || message[2] != A1RLQ_DataConstants.ACK)
	               	{
	               		...
	               		rcvFrame.append(message);
		            	if(isContinue(message[A1RLQ_DataConstants.POS_LEN])){
		            		frame = new A1RLQ_RequestDataFrame(new BYTE(), new BYTE(A1RLQ_DataConstants.CB_CONTINUEREAD), 0,0, null);
		            		isContinue = true;
		            	}
	               	}
	            }else{
	...
	/**
	 * 
	 * Meter로 부터 받은 데이터 길이값에서
	 * 첫번째 비트가 0이면 64바이트 이상이므로 
	 * continue로 받아야하고 1이면 64바이트 이내
	 * 이것 여부를 판단.
	 * @param len
	 * @return
	 */
