 ...
 *
 */
public class EMnVInverterLogAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVInverterLogAction.class);
	private EMnVCommandSubFrameType commandType;

	public EMnVInverterLogAction(EMnVCommandSubFrameType commandType) {
	...
	public void execute(EMnVGeneralFramePayLoad gFramePayload, IoSession session, EMnVGeneralDataFrame generalDataFrame) throws Exception {
		...
		case COMMAND_REQUEST:
            ...
            logger.debug("[{}][COMMAND_REQUEST][TRAN_ID={}] - DEVICE={}, COMMAND={}, STATE={}, ASYNC_SIZE={}"
            		, new Object[]{ActionCommandType.ON_DEMAND_INVERTER_LOG.name(), aDevice.getTransactionId(), aDevice.getModemId(), aDevice.getCommand(), TR_STATE.Waiting.getCode(), acllist.size ()});
            if (acllist.size() > 0) {
            	/*
                ...
                String modemPortNumber = "";
                int lpIndex = 0;
                
                for(AsyncCommandParam param : acplist){
                	if(param.getParamType().equals("modem_port")){
                		modemPortNumber = param.getParamValue();
                	}else if(param.getParamType().equals("lp_idx")){
                		lpIndex = Integer.parseInt(param.getParamValue().trim());
                	}
                }
                
                payLoad = new EMnVCommandFramePayLoad(EMnVCommandSubFrameType.COMMAND_REQUEST);
                payLoad.setCommandType(ActionCommandType.ON_DEMAND_INVERTER_LOG);
                payLoad.setRwsType(RWSType.R);
                payLoad.setErrorBitType(ErrorBitType.SUCCESS);
                payLoad.setModemPort(Hex.encode(modemPortNumber));
                payLoad.setLpIndex(DataUtil.getByteToInt(lpIndex));
                
                generalDataFrame.setGeneralFramePayLoad(payLoad);
                
                logger.info("");
                logger.info("[{}][COMMAND_REQUEST][TRAN_ID={}][remote={}] modem_port_number={}, lp_idx={}"
                		, new Object[]{ActionCommandType.ON_DEMAND_INVERTER_LOG.name(), aDevice.getTransactionId(), session.getRemoteAddress(), modemPortNumber, lpIndex});
		...
		case COMMAND_RESPONSE:
			...
			case SUCCESS:
				logger.info("[ON_DEMAND_INVERTER_LOG][COMMAND_RESPONSE][{}][TRNX_ID={}] Ondemand Success !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
			...
			case NOT_SUPPORTED_COMMAND:
				logger.error("[ON_DEMAND_INVERTER_LOG][COMMAND_RESPONSE][{}][TRNX_ID={}] Ondemand Fail !!! - NOT_SUPPORTED_COMMAND ", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case NOT_ACCESS_USER:
				logger.error("[ON_DEMAND_INVERTER_LOG][COMMAND_RESPONSE][{}][TRNX_ID={}] Ondemand Fail !!! - NOT_ACCESS_USER", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case FAIL:
				logger.error("[ON_DEMAND_INVERTER_LOG][COMMAND_RESPONSE][{}][TRNX_ID={}] Ondemand Fail !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
	                for (int i = 0; i < acLogList.size(); i++) {
	                	AsyncCommandLog acl = acLogList.get(i);
					...
					logger.info("[ON_DEMAND_INVERTER_LOG][COMMAND_RESPONSE][{}][TRNX_ID={}], TR_STATE Change - Terminate", aDevice.getTransactionId(), aDevice.getModemId());
	            }
				break;
			case UNKNOWN:
