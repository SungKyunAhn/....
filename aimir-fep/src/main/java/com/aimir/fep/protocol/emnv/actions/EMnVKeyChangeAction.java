 ...
 *
 */
public class EMnVKeyChangeAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVKeyChangeAction.class);
	private EMnVCommandSubFrameType commandType;
	private RWSType rwsType;
	private ActionCommandType actionCommandType = ActionCommandType.KEY_CHANGE;
	
	public EMnVKeyChangeAction(EMnVCommandSubFrameType commandType) {
            ...
            if (acllist.size() > 0) {
               ...
               String staticKey = null;
                for(AsyncCommandParam param : acplist){
                	if(param.getParamType().equals("key")){
                		staticKey = param.getParamValue();
                	}
                }
                ...
                payLoad.setStaticKey(staticKey.getBytes());
                
                HashMap<String, Object> params = new HashMap<String, Object>();
                params.put("w_static_key", staticKey);
                aDevice.setParams(params);
                
                generalDataFrame.setGeneralFramePayLoad(payLoad);
                
                logger.info("");
                logger.info("#### [{}][COMMAND_REQUEST][TRAN_ID={}][remote={}] RWS_Type={}, key={}"
                		, new Object[]{actionCommandType.name(), aDevice.getTransactionId()
                				, session.getRemoteAddress(), rwsType, staticKey});
			...
			case SUCCESS:
				logger.info("[{}][COMMAND_RESPONSE][{}][TRNX_ID={}][{}-{}] Success !!!"
						, new Object[]{actionCommandType.name(), aDevice.getModemId(), aDevice.getTransactionId(), payLoad.getRwsType().getDesc()
								, aDevice.getParams().get("w_static_key")});
	            if (acLogList.size() > 0) {
