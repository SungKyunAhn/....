 ...
 *
 */
public class EMnVServerPortAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVServerPortAction.class);
	private EMnVCommandSubFrameType commandType;
	private RWSType rwsType;
	private ActionCommandType actionCommandType = ActionCommandType.SERVER_PORT;

	public EMnVServerPortAction(EMnVCommandSubFrameType commandType) {
	...
	public void execute(EMnVGeneralFramePayLoad gFramePayload, IoSession session, EMnVGeneralDataFrame generalDataFrame) throws Exception {
            ...
            if (acllist.size() > 0) {
               ...
               String serverPort = null;
                for(AsyncCommandParam param : acplist){
                	if(param.getParamType().equals("server_port")){
                		serverPort = param.getParamValue();
                	}
                }
                ...
                if(rwsType == RWSType.W){
                    payLoad.setServerPort(DataUtil.get2ByteToInt(serverPort));
                    HashMap<String, Object> params = new HashMap<String, Object>();
                    params.put("w_server_port", serverPort);
                    aDevice.setParams(params);
                }
                generalDataFrame.setGeneralFramePayLoad(payLoad);
                
                logger.info("");
                logger.info("#### [{}][COMMAND_REQUEST][TRAN_ID={}][remote={}] RWS_Type={}, server_port={}"
                		, new Object[]{actionCommandType.name(), aDevice.getTransactionId()
                				, session.getRemoteAddress(), rwsType, serverPort});
		...
		case COMMAND_RESPONSE:
			...
			case SUCCESS:
				logger.info("[{}][COMMAND_RESPONSE][{}][TRNX_ID={}][{}-{}] Success !!!"
						, new Object[]{actionCommandType.name(), aDevice.getModemId(), aDevice.getTransactionId(), payLoad.getRwsType().getDesc()
								, rwsType == RWSType.W ? aDevice.getParams().get("w_server_port") : DataUtil.getIntToBytes(payLoad.getServerPort())});
	            if (acLogList.size() > 0) {
	                ...
	                if(rwsType == RWSType.R){
	    	    		...
	    	    		asyncCommandParam.setParamType("SERVER_PORT");	
	    	    		asyncCommandParam.setParamValue(String.valueOf(DataUtil.getIntToBytes(payLoad.getServerPort())));
			...
			case NOT_SUPPORTED_COMMAND:
				logger.error("[SERVER_PORT][COMMAND_RESPONSE][{}][TRNX_ID={}] Meter Timesync Fail !!! - NOT_SUPPORTED_COMMAND ", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case NOT_ACCESS_USER:
				logger.error("[SERVER_PORT][COMMAND_RESPONSE][{}][TRNX_ID={}] Meter Timesync Fail !!! - NOT_ACCESS_USER", aDevice.getModemId(), aDevice.getTransactionId());
				break;
			case FAIL:
				logger.error("[SERVER_PORT][COMMAND_RESPONSE][{}][TRNX_ID={}] Meter Timesync Fail !!!", aDevice.getModemId(), aDevice.getTransactionId());
	            if (acLogList.size() > 0) {
	                for (int i = 0; i < acLogList.size(); i++) {
	                	AsyncCommandLog acl = acLogList.get(i);
