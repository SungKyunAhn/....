 ...
 */
@Component
public class EV_205_2_0_Action implements EV_Action
{
    private static Log log = LogFactory.getLog(EV_205_2_0_Action.class);
    ...
    {
        log.debug("EV_205_2_0_Action : EventName[eventCommZMUError] "
        ...
        try { 
            ...
            log.debug("EV_205_2_0_Action : byteEntry[" + tmpval + "]");
            nReason = Integer.parseInt(tmpval);
        }catch(Exception ex) { log.error(ex,ex); }

        // Log
        log.debug("EV_205_2_0_Action : modemId[" + modemId + "]"
                +" nReason["+nReason+"] ");
        ...
        try {
            ...
            if (modem != null)
            {
                ...
                event.setActivatorType(modem.getDeviceSerial());
            }
            else {
        ...
        log.debug("Communication ZMU Failure Event Action Compelte");
    }
}
