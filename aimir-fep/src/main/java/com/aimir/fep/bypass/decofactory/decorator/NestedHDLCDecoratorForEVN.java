 ...
 *
 */
public class NestedHDLCDecoratorForEVN extends NestFrameDecorator {
	private static Logger logger = LoggerFactory.getLogger(NestedHDLCDecoratorForEVN.class);
	 ...
	 * @param nestedFrame
	 */
	public NestedHDLCDecoratorForEVN(INestedFrame nestedFrame) {
		super(nestedFrame);
	}
	...
	public byte[] encode(HdlcObjectType hdlcType, Procedure procedure, HashMap<String, Object> param) {
		...
		try {
			...
			if (hdlcType == HdlcObjectType.SNRM) {
				...
				gdDLMSFrame = DataUtil.append(gdDLMSFrame, new byte[] { hdlcType.getBytes() });
			} else 	if (hdlcType == HdlcObjectType.RR) {
	...
	public boolean decode(IoSession session, byte[] frame, Procedure procedure) {
		...
		try {
			...
			logger.debug("[HDLC] CONTROL = [{}]", Hex.decode(control));
