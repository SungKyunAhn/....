...
public class DLMSImageTransfer extends DLMSObject implements DLMSBase
{
    ...
    */
    public DLMSImageTransfer()
    {
        ...
        DLMSImageActivateInfo info = new DLMSImageActivateInfo();
        info.Size = 0;
        info.Signature = "";
        info.Identification = "";
        ImageActivateInfo = new DLMSImageActivateInfo[] { info };
    ...
    /**  
     ...
     @param ln Logican Name of the object.
    */
    public DLMSImageTransfer(String ln)
    {
    ...
    */
    public DLMSImageTransfer(String ln, int sn)
    {
    ...
    */
    ...
    public final void setImageActivateInfo(DLMSImageActivateInfo[] value)
    {
    ...
    public byte[][] invoke(Object sender, int index, Object parameters)
    {
        ...
        if (index == 1)
        {
            ...
            List<DLMSImageActivateInfo> list = new ArrayList<DLMSImageActivateInfo>();
            ...
            list.add(item);
            ImageActivateInfo = list.toArray(new DLMSImageActivateInfo[list.size()]);
    ...
    public Object getValue(int index, int selector, Object parameters)
    {
        ...
        if (index == 7)
        {
            ...
            try
            {
                for(DLMSImageActivateInfo it : ImageActivateInfo)
                {
                    data.write((byte)DataType.STRUCTURE.getValue());                    
                    data.write((byte)3);//Item count.
                    Common.setData(data, DataType.UINT32, it.getSize());
                    Common.setData(data, DataType.OCTET_STRING, Common.getBytes(it.getIdentification()));
                    String tmp = it.getSignature();
                    if (tmp != null)
                    {
                        Common.setData(data, DataType.OCTET_STRING, Common.getBytes(it.getSignature()));
                    }
                    else
                    {
                        Common.setData(data, DataType.OCTET_STRING, null);
                    }
                }
            }
            catch(Exception ex)
    ...
    public void setValue(int index, Object value)
    {
        ...
        {
            ImageActivateInfo = new DLMSImageActivateInfo[0];
            if (value != null)
            {        
                List<DLMSImageActivateInfo> list = new ArrayList<DLMSImageActivateInfo>();
     ...
     public byte[][] imageTransferInitiate(DLMSClient client, String imageIdentifier, long imageSize)
    {
        if (ImageBlockSize == 0)
        {
        ...
        try
        {            
            Common.setData(data, DataType.OCTET_STRING, Common.getBytes(imageIdentifier));
            Common.setData(data, DataType.UINT32, imageSize);
        }
        catch(Exception ex)
        {
            throw new RuntimeException(ex.getMessage());
    ...
    public byte[][] imageBlockTransfer(DLMSClient client, byte[] imageBlockValue, int[] imageBlockCount)
    {
        ...
        try
        {
            for (int pos = 0; pos != cnt; ++pos)
            {
                ...
                data.write(2);
                Common.setData(data, DataType.UINT32, pos);
                byte[] tmp;
                ...
                Common.setData(data, DataType.OCTET_STRING, tmp);
    ...
    public byte[][] imageVerify(DLMSClient client)
    {
        return client.method(this, 3, 0, DataType.INT8);
    }

    public byte[][] imageActivate(DLMSClient client)
    {
