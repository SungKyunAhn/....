...
{       
    ...
    */
    ...
    private static byte[] EncryptAesGcm(Command command, Security security, long FrameCounter, byte[] systemTitle, byte[] BlockCipherKey, byte[] AuthenticationKey, byte[] plainText, CountType type, Object[] countTag)
    {
        try
        {
            ...
            DLMSChipperingStream gcm = new DLMSChipperingStream(security, true, BlockCipherKey, aad, GetNonse(FrameCounter, systemTitle), null);
            ...
            if (type == CountType.PACKET)
            {
                ...
                Common.setObjectCount(data.size(), tmp2);
                data.writeTo(tmp2);
    ...
    */
    static byte[] DecryptAesGcm(byte[] cryptedText, byte[] systemTitle, byte[] blockCipherKey, byte[] authenticationKey)
    {
        ...
        int len = Common.getObjectCount(cryptedText, pos);
        ...
        int[] index = new int[1];
        long frameCounter = (long) Common.getUInt32(FrameCounterData, index);
        ...
        if (security == Security.AUTHENTICATION)
        {
            ...
            countTag = (byte[]) tmp[0];
            if (!DLMSChipperingStream.TagsEquals(tag, countTag))
            {
        ...
        DLMSChipperingStream gcm = new DLMSChipperingStream(security, false, blockCipherKey, aad, GetNonse(frameCounter, systemTitle), tag);
