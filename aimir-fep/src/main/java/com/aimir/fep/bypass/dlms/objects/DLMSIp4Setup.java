...
public class DLMSIp4Setup extends DLMSObject implements DLMSBase
{
    ...
    */
    public DLMSIp4Setup(String ln, int sn)
    {
    ...
    public final void setIPOptions(DLMSIp4SetupIpOption[] value)
    {
        IPOptions = value;
    }
    ...
    public Object getValue(int index, int selector, Object parameters)
    {
        ...
        if (index == 5)
        {
            ...
            {
                Common.setObjectCount(IPOptions.length, data);
                for (DLMSIp4SetupIpOption it : IPOptions)
                {
                    ...
                    try 
                    {
                        ...
                        Common.setData(data, DataType.OCTET_STRING, it.getData());                        
                    } 
                    catch (Exception ex)
                    {
                        Logger.getLogger(DLMSIp4Setup.class.getName()).log(Level.SEVERE, null, ex);
                    }
                }
            }
    ...
    public void setValue(int index, Object value)
    {
        ...
        else if (index == 2)
        {
            ...
            {
                this.setDataLinkLayerReference(DLMSClient.changeType((byte[])value, DataType.OCTET_STRING).toString());
            }
        }
        else if (index == 3)
        {
        ...
        {
            java.util.ArrayList<DLMSIp4SetupIpOption> data = new java.util.ArrayList<DLMSIp4SetupIpOption>();
            if (value != null)
            {
                for (Object it : (Object[])value)
                {
                    DLMSIp4SetupIpOption item = new DLMSIp4SetupIpOption();
                    item.setType(DLMSIp4SetupIpOptionType.forValue(((Number)(Array.get(it, 1))).intValue()));
                    item.setLength(((Number)(Array.get(it, 1))).shortValue());
                    item.setData((byte[]) Array.get(it, 2));
                    data.add(item);
                }
            }
            setIPOptions(data.toArray(new DLMSIp4SetupIpOption[data.size()]));
        }
        else if (index == 6)
        {
