...
/** 
 DLMSServer implements methods to implement DLMS/COSEM meter/proxy.
*/
abstract public class DLMSServerBase
{
    int LastCommand;
    ByteArrayOutputStream ReceivedData = new ByteArrayOutputStream();
    private DLMSObjectCollection m_Items;
    DLMS m_Base = new DLMS(true);	
    ByteArrayOutputStream ReceivedFrame = new ByteArrayOutputStream();
    ArrayList<byte[]> SendData = new ArrayList<byte[]>();
    int FrameIndex = 0;
    boolean Initialized = false;
    private TreeMap<Integer, DLMSObject> SortedItems = new TreeMap<Integer, DLMSObject>();
    ...
    */
    ...
    /** 
     Constructor.
    */
    public DLMSServerBase(boolean logicalNameReferencing)
    {
        m_Base = new DLMS(true);
        m_Items = new DLMSObjectCollection(this);        
    ...
    */
    ...
    {
        return ((Number)Manufacturer.countServerAddress(HDLCAddressType.SERIAL_NUMBER, serialNumber, 0)).intValue();
    }
    ...
    */
    public final Object countServerID(Object physicalAddress, int LogicalAddress)
    {
        ...
        return Manufacturer.countServerAddress(HDLCAddressType.DEFAULT, physicalAddress, LogicalAddress);
    }
    ...
    */
    ...
    public final void setItems(DLMSObjectCollection value)
    {
        m_Items = value;
    }
    ...
    */
    ...
    /** 
     ...
     @see DLMSServerBase#getDLMSVersion
     @see DLMSServerBase#getUseLogicalNameReferencing
    */
     ...
     * @see DLMSServerBase#getDLMSVersion
     * @see DLMSServerBase#getMaxReceivePDUSize
    */
    ...
    */
    public final java.util.ArrayList<DLMSAuthentication> getAuthentications()
    {
    ...
    public final void setInterfaceType(InterfaceType value)
    {
        ...
        if (changed || Authentications == null)
        {
            ...
            if (value == InterfaceType.GENERAL)
            {
                ...
                {
                    Authentications = new ArrayList<DLMSAuthentication>();
                }
                else
                {
                ...
                try
                {
                    Authentications.add(new DLMSAuthentication(Authentication.NONE, "".getBytes("ASCII"), (byte) 0x10));
                    Authentications.add(new DLMSAuthentication(Authentication.LOW, "GuruxLow".getBytes("ASCII"), (byte) 0x20));
                    Authentications.add(new DLMSAuthentication(Authentication.HIGH, "GuruxHigh".getBytes("ASCII"), (byte) 0x40));        
                    Authentications.add(new DLMSAuthentication(Authentication.HIGH_MD5, "GuruxHighMD5".getBytes("ASCII"), (byte) 0x40));        
                    Authentications.add(new DLMSAuthentication(Authentication.HIGH_SHA1, "GuruxHighSHA1".getBytes("ASCII"), (byte) 0x40));        
                    Authentications.add(new DLMSAuthentication(Authentication.HIGH_GMAC, "GuruxHighGMAC".getBytes("ASCII"), (byte) 0x40));        
                }
            ...
            else
            {
                ...
                try
                {
                    Authentications.add(new DLMSAuthentication(Authentication.NONE, "".getBytes("ASCII"), (short) 0x10));
                    Authentications.add(new DLMSAuthentication(Authentication.LOW, "GuruxLow".getBytes("ASCII"), (short) 0x20));
                    Authentications.add(new DLMSAuthentication(Authentication.HIGH, "GuruxHigh".getBytes("ASCII"), (short) 0x40));        
                    Authentications.add(new DLMSAuthentication(Authentication.HIGH_MD5, "GuruxHighMD5".getBytes("ASCII"), (short) 0x40));        
                    Authentications.add(new DLMSAuthentication(Authentication.HIGH_SHA1, "GuruxHighSHA1".getBytes("ASCII"), (short) 0x40));        
                    Authentications.add(new DLMSAuthentication(Authentication.HIGH_GMAC, "GuruxHighGMAC".getBytes("ASCII"), (byte) 0x40));        
                }
                catch(UnsupportedEncodingException ex)
                {
    ...
    */
    private void getCommand(int cmd, byte[] data, ObjectType[] type, List<Object> names, int[] attributeIndex, int selector[], Object[] parameters)
    {
        ...
        {
            type[0] = ObjectType.forValue(Common.getUInt16(data, index));
            java.nio.ByteBuffer tmp = java.nio.ByteBuffer.wrap(Common.rawData(data, index, 6));
            ...
            if (data.length - index[0] != 0)
            {
                ...
                parameters[0] = Common.getData(data, index, 0, a, b, dt, c);
            } 
        }
        else
        {
            ...
            if (cmd == Command.ReadRequest.getValue())
            {
                for (int pos = 0; pos != cnt; ++pos)
                {
                    ...
                    if (tp == 2)
                    {
                        names.add(Common.getUInt16(data, index));
                    }
                    else if (tp == 4)
                    {
                        names.add(Common.getUInt16(data, index));
            ...
            else if (cmd == Command.WriteRequest.getValue())
            {
                ...
                //Get data count
                cnt = Common.getObjectCount(data, index);
                for (int pos = 0; pos != cnt; ++pos)
                {
    ...
    */
    public final void initialize()
    {
        ...
        if (SortedItems.size() != getItems().size())
        {
            ...
            {
                DLMSObject it = m_Items.get(pos);
                if (it.getLogicalName() == null)
                {
                    throw new RuntimeException("Invalid Logical Name.");
                }
                if (it instanceof DLMSProfileGeneric)
                {
                    DLMSProfileGeneric pg = (DLMSProfileGeneric)((it instanceof DLMSProfileGeneric) ? it : null);
                    pg.setServer(this);
                    ...
                    {
                        new ProfileGenericUpdater(this, pg).start();
                    }
                }
                else if ((it instanceof DLMSAssociationShortName && 
                        !this.getUseLogicalNameReferencing()) || 
                        (it instanceof DLMSAssociationLogicalName && 
                        this.getUseLogicalNameReferencing()))
                {
                    association = true;
                }
                else if (!(it instanceof DLMSBase)) //Remove unsupported items.
                {
            ...
            if (!association)
            {
                ...
                {
                    DLMSAssociationLogicalName ln = new DLMSAssociationLogicalName();
                    for(DLMSObject it : m_Items)
                    {
                ...
                {
                    DLMSAssociationShortName sn = new DLMSAssociationShortName();
                    for(DLMSObject it : m_Items)
                    {
    ...
    */
    private byte[] handleAARQRequest(byte[] data) throws Exception
    {
        ...
        {
            DLMSAuthentication auth = null;
            for (DLMSAuthentication it : getAuthentications())
            {
            ...
            else if (aarq.authentication != Authentication.NONE)
            {
                ...
                {
                    m_Base.StoCChallenge = DLMS.generateChallenge();
    ...
    {
        byte[] tmp = Common.getAsByteArray(data);
    ...
    */
    private Object getAdd(byte[] buff, int index[])
    {
        ...
        {
            return Common.getUInt16(buff, index);
        }
        else if (size == 4)
        {
            return Common.getUInt32(buff, index);
        }
    ...
    */
            ...
            throws UnsupportedEncodingException
    {
        ...
        if (getInterfaceType() != InterfaceType.NET)
        {
            ...
            {
                if (buff[index[0]] == Common.HDLCFrameStartEnd)
                {
            ...
            if (MoreData.isEmpty() && 
                buff[FrameLen + PacketStartID + 1] != Common.HDLCFrameStartEnd)
            {
                throw new DLMSException("Invalid data format.");
            }
        ...
        else
        {
            ...
            clientId[0] = (short) Common.getUInt16(buff, index);
            serverId[0] = (short) Common.getUInt16(buff, index);
        }
        return true;
    }
    ...
    */
    public final byte[] handleRequest(byte[] buff)
    {
        ...
        try
        {
            ...
            else if (ret.contains(RequestTypes.DATABLOCK))
            {
                ...
                ++FrameIndex;
                int[] index = new int[1];
                int BlockIndex = (int)Common.getUInt32(allData, index);
                return SendData.get(FrameIndex);
            }
            ...
            else if (command[0] == Command.WriteRequest.getValue())
            {
                ...
                int sn = (Integer)names.get(0);
                for (Map.Entry<Integer, DLMSObject> it : SortedItems.entrySet())                    
                {
                    int aCnt = ((DLMSBase) it.getValue()).getAttributeCount();
                    if (sn >= it.getKey() && sn <= (it.getKey() + (8 * aCnt)))
                    {
                        ...
                        if (value instanceof Byte[])
                        {
                            ...
                            if (tp != DataType.NONE)
                            {
                                value[0] = DLMSClient.changeType((byte[])value[0], tp);
                            }
                        }
                        ...
                        if (!e.getHandled())
                        {
                            ((DLMSBase) item).setValue(attributeIndex, value[0]);
                        }
                        //Return OK.
            ...
            else if (command[0] == Command.SetRequest.getValue())
            {
                ...
                if (item != null)
                {
                    ...
                    if (!e.getHandled())
                    {
                        ((DLMSBase) item).setValue(attributeIndex[0], value[0]);
                    }
                    //Return OK.
                    SendData.addAll(Arrays.asList(acknowledge(getUseLogicalNameReferencing() ? Command.SetResponse : Command.WriteResponse, 0)));
                    return SendData.get(FrameIndex);
            ...
            else if (command[0] == Command.ReadRequest.getValue() && !getUseLogicalNameReferencing())
            {
                ...
                for (Map.Entry<Integer, DLMSObject> it : SortedItems.entrySet())
                {
                    ...
                    //If action.
                    else if (sn >= it.getKey() + aCnt && ((DLMSBase) it.getValue()).getMethodCount() != 0)
                    {
                        //Convert DLMS data to object type.
                        int[] value2 = new int[1], count = new int[1];
                        DLMS.getActionInfo(it.getValue().getObjectType(), value2, count);
                        if (sn <= it.getKey() + value2[0] + (8 * count[0]))//If action
                        {
                            ...
                            if (!e.getHandled())
                            {
                                byte[][] reply = ((DLMSBase) item).invoke(this, attributeIndex, e.getValue());
                                if (reply != null)
                                {
    ...
    private byte[] getValue(Object name, DLMSObject item, int index, int selector, Object parameters)
            throws RuntimeException, UnsupportedEncodingException, 
            ParseException, IOException
    {
        Object value;        
        DLMSBase base = (DLMSBase) item;
        if (base != null)
        {            
    ...
    */
            ...
            ParseException, IOException
    {
        ...
        Common.setData(data, type, value);        
