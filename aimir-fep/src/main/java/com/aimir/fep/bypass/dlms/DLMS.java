...
class DLMS
{
    ...
    */
    ...
    static DLMSObject createObject(ObjectType type)
    {
    ...
    /** 
     Constructor.
    */
    public DLMS(boolean server)
    {
        m_Priority = Priority.HIGH;
        m_InvokeID = 1;        
        try 
        {
            Ciphering = new Ciphering("ABCDEFGH".getBytes("ASCII"));
        }
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    int base_address = Common.intValue(name);
    ...
    */
    ...
    public final void setLimits(DLMSLimits value)
    {
        privateLimits = value;
    }
    ...
    */
    public final byte[] receiverReady(RequestTypes type)
    {
        ...
        if (this.getInterfaceType() == InterfaceType.GENERAL)
        {
            if (getServer())
            {
                buff.put(Common.LLCReplyBytes);
            }
            else
            {
                buff.put(Common.LLCSendBytes);
            }
        }
        //Get request normal
        buff.put((byte) 0xC0);
    ...
    */
            ...
            Object[] value, DataType[] type)
    {
        ...
        value[0] = Common.getData(buff, index, action.getValue(), total, read, type, cache);
        cacheIndex = cache[0];
        if (getUseCache())
        {
                        ...
                        int[] index, int count, Command cmd, int resultChoice)
    {
           ...
           !(Ciphering.getSecurity() != Security.NONE && frameSequence != -1))
        {
            try
            {
                if (getServer())
                {
                    tmp.write(Common.LLCReplyBytes);
                }
                else
                {
                    tmp.write(Common.LLCSendBytes);
                }
            }
            catch(Exception ex)
            {
        ...
        if (cmd != Command.None && this.getUseLogicalNameReferencing())
        {
            ...
            if (moreBlocks)
            {
                ...
                tmp.write((byte) 0);
                Common.setObjectCount(count, tmp);
            }
        }
        ...
        if (Ciphering.getSecurity() != Security.NONE && frameSequence != -1)
        {
            ...
            packet = java.nio.ByteBuffer.wrap(DLMSChippering.EncryptAesGcm(gloCmd, Ciphering.getSecurity(), 
        ...
        else
        {
            ...
            {
                dataSize = Common.intValue(getLimits().getMaxInfoTX());
            }
            else
            {
                dataSize = Common.intValue(getLimits().getMaxInfoRX());
            }
        }
        if (count + index[0] > packet.limit())
        {
    ...
    */
    ...
    public final void setLNSettings(DLMSLNSettings value)
    {
    ...
    */
    ...
    public final void setSNSettings(DLMSSNSettings value)
    {
    ...
    final byte[] addFrame(byte Type, boolean moreFrames, byte[] data, int index, int count)
    {
        ...
        else
        {
            if (this.getGenerateFrame())
            {
                ...
                buff.put(Common.HDLCFrameStartEnd);
            }
            //Frame type
            ...
            if (count > 0)
            {
                ...
                crc = FCS16.countFCS16(buff, start, cnt);
            ...
            if (this.getGenerateFrame())
            {
                ...
                crc = FCS16.countFCS16(buff, 1, buff.position() - 1);
                buff.putShort((short) crc);
                //EOP
                buff.put(Common.HDLCFrameStartEnd);
            }
    ...
    */
    private boolean checkLLCBytes(java.nio.ByteBuffer buff)
    {
        if (getInterfaceType() == InterfaceType.GENERAL)
        {
            if (getServer())
            {
                return Common.compare(buff, Common.LLCSendBytes);
            }
            return Common.compare(buff, Common.LLCReplyBytes);
        }
        return false;
    }
    ...
    */
            ...
            boolean[] wrongCrc, int[] command)
    {
        ...
        if (getInterfaceType() != InterfaceType.NET)
        {
            ...
            {
                if (buff.get() == Common.HDLCFrameStartEnd)
                {
            ...
            if (MoreData.isEmpty() && 
                    buff.get(FrameLen + PacketStartID + 1) != Common.HDLCFrameStartEnd)                
            {
                throw new DLMSException("Invalid data format.");
            }
        }
        ...
        if (!Common.compare(buff, ClientBuff))
        {
            //If echo.
            if (getInterfaceType() != InterfaceType.NET && FrameLen != 0)
            {
                if (Common.compare(buff, ServerBuff) && 
                    Common.compare(buff, ClientBuff))
            ...
            throw new DLMSException("Source addresses do not match. It is " + 
                    getAddress(buff, tmp, ClientBuff.length) + ". It should be " + this.getClientID() + ".");
        }
        if (!Common.compare(buff, ServerBuff))
        {
            int tmp[] = new int[]{buff.position()};
            throw new DLMSException("Destination addresses do not match. It is " + 
        ...
        if (getInterfaceType() != InterfaceType.NET)
        {
            ...
            if (frame[0] == FrameType.SNRM.getValue() || frame[0] == FrameType.Disconnect.getValue())
            {
                ...
                int crcCount = FCS16.countFCS16(buff, PacketStartID + 1, len - PacketStartID - 4);
                if (crcRead != crcCount)
                {
                    packetFull[0] = false;
                    wrongCrc[0] = true;
            ...
            else
            {
                ...
                int crcCount = FCS16.countFCS16(buff, PacketStartID + 1, buff.position() - PacketStartID - 1);
                int crcRead = buff.getShort() & 0xFFFF;
                if (crcRead != crcCount)
                {
                ...
                //Check that CRC match.
                crcCount = FCS16.countFCS16(buff, PacketStartID + 1, len - PacketStartID - 4);
                crcRead = buff.getShort(len - 3) & 0xFFFF;
                if (crcRead != crcCount)
                {
                    System.out.println(Common.toHex(buff.array()));
                    wrongCrc[0] = true;
                    MoreData.clear();
                    return MoreData;
                }
    ...
    public final byte[] GetCipheredData(byte[] data, Ciphering ciphering)
    {
        return DLMSChippering.DecryptAesGcm(data, ciphering.getSystemTitle(), 
    ...
    public final byte[] decrypt(byte[] buff, Command[] command)
    {
        ...
        int len = Common.getObjectCount(buff, index);
    ...
    private boolean getLNData(java.nio.ByteBuffer buff, int[] pError, java.util.Set<RequestTypes> MoreData, int command)
    {
        ...
        if (res == 0xD8)
        {
            ...
            throw new DLMSException(stateError.toString() + " " + serviceError.toString());
        }
            ...
            else
            {
                ...
                else
                {
                    if (AttributeID == 0x02)
                    {
                        ...
                        Common.getObjectCount(buff, index);
