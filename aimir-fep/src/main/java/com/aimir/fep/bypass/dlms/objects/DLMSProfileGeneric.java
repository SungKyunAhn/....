...
public class DLMSProfileGeneric extends DLMSObject implements DLMSBase
{
    ...
    */
    public DLMSProfileGeneric(String ln)
    {
        this(ln, (short) 0);
    }
    ...
    */
    public DLMSProfileGeneric(String ln, int sn)
    {
        super(ObjectType.PROFILE_GENERIC, ln, sn);
        setAccessSelector(AccessRange.Last);
        m_CaptureObjects = new ArrayList<AbstractMap.SimpleEntry<DLMSObject, DLMSCaptureObject>>();
    }   
    ...
    */
    ...
    public final void setServer(DLMSServerBase value)
    {
        Server = value;
    }
    ...
    */
     ...
     */
    public void addCaptureObject(DLMSObject item, int attributeIndex, int dataIndex)
    {
        if (item == null)            
        {
        ...
        DLMSCaptureObject co = new DLMSCaptureObject(attributeIndex, dataIndex);
        m_CaptureObjects.add(new AbstractMap.SimpleEntry<DLMSObject, DLMSCaptureObject>(item, co));        
    }
    /** 
     Captured Objects.
    */
    ...
    {
        List<DLMSObject> objects = new ArrayList<DLMSObject>();
        for(AbstractMap.SimpleEntry<DLMSObject, DLMSCaptureObject> it : m_CaptureObjects)
        {
            objects.add(it.getKey());
        }
        return objects.toArray(new DLMSObject[objects.size()]);
    }
    ...
    */
    ...
    public final void setSortObject(DLMSObject value)
    {
        m_SortObject = value;
    }
    ...
    private byte[] getColumns()
    {
        try
        {
            ...
            for (AbstractMap.SimpleEntry<DLMSObject, DLMSCaptureObject> it : m_CaptureObjects)
            {
                ...
                Common.setData(data, DataType.UINT16, it.getKey().getObjectType().getValue()); //ClassID
                Common.setData(data, DataType.OCTET_STRING, it.getKey().getLogicalName()); //LN
                Common.setData(data, DataType.INT8, it.getValue().getAttributeIndex()); //Attribute Index
                Common.setData(data, DataType.UINT16, it.getValue().getDataIndex()); //Data Index
            }
            return data.array();
        }
        catch(Exception ex)
    ...
    */
    private byte[] getData(Object[] table)            
    {
        ...
        Common.setObjectCount(Array.getLength(table), data);
        ...
        for (Object row : table)
        {
            ...
            Common.setObjectCount(Array.getLength(items), data);
            pos = -1;
    ...
    {                        
        ...
        if (selector == 1) //Read by range
        {
            ...
            java.util.Date start = ((DLMSDateTime)Common.getData((byte[])arr[1], pos, 0, a, b, dt, c)).getValue();
            c[0] = pos[0] = 0;
            java.util.Date end = ((DLMSDateTime)Common.getData((byte[])arr[2], pos, 0, a, b, dt, c)).getValue();
            for (Object row : getBuffer())
            {
                java.util.Date tm;
                Object tmp = ((Object[]) row)[0];
                if (tmp instanceof DLMSDateTime)
                {
                    tm = ((DLMSDateTime)tmp).getValue();    
                }
                else
                {
    ...
    public Object getValue(int index, int selector, Object parameters)
    {
        ...
        if (index == 6)
        {
            ...
            try
            {
                ...
                {
                    Common.setData(data, DataType.UINT16, 0); //ClassID
                    Common.setData(data, DataType.OCTET_STRING, new byte[6]); //LN
                    Common.setData(data, DataType.INT8, 0); //Selected Attribute Index
                    Common.setData(data, DataType.UINT16, 0); //Selected Data Index
                }
                else
                {
                    Common.setData(data, DataType.UINT16, m_SortObject.getObjectType().getValue()); //ClassID
                    Common.setData(data, DataType.OCTET_STRING, m_SortObject.getLogicalName()); //LN
                    Common.setData(data, DataType.INT8, SortObjectAttributeIndex); //Attribute Index
                    Common.setData(data, DataType.UINT16, SortObjectDataIndex); //Data Index
                }
            }
            catch(Exception ex)
            {
    ...
    public void setValue(int index, Object value)
    {
            ...
            if (value != null)
            {
                ...
                for(Object row : (Object[]) value)
                {
                    ...
                    for(int a = 0; a < Array.getLength(row); ++a)
                    {
                        ...
                        {
                            data = DLMSClient.changeType((byte[]) data, type);
                            if (data instanceof DLMSDateTime)
                            {
                                DLMSDateTime dt = (DLMSDateTime)data;                                
                        ...
                        else if (type == DataType.DATETIME && data == null && CapturePeriod != 0)
                        {
                            ...
                            {
                                lastDate.setTime(((DLMSDateTime)m_Buffer.get(m_Buffer.size() - 1)[pos]).getValue());
                            }
                            if (lastDate.getTimeInMillis() != 0)
                            {
                                lastDate.add(java.util.Calendar.SECOND, CapturePeriod);                                            
                                Array.set(row, a, new DLMSDateTime(lastDate.getTime()));
                            }
                        }
                        AbstractMap.SimpleEntry<DLMSObject, DLMSCaptureObject> item = m_CaptureObjects.get(pos);
                        if (item.getKey() instanceof DLMSRegister && item.getValue().AttributeIndex == 2)
                        {
                            double scaler = ((DLMSRegister) item.getKey()).getScaler();
                            if (scaler != 1)
                            {
                                try
                                {
        ...
        else if (index == 3)
        {
            ...
            if (value != null)
            {
                for (Object it : (Object[]) value)
                {
                    ...
                    String ln = DLMSObject.toLogicalName((byte[])tmp[1]);
                    DLMSObject obj = null;
                    if (getParent() != null)
                    {
        ...
        else if (index == 6)
        {
            ...
            else
            {
                ...
                if(m_SortObject == null)
                {                        
                    m_SortObject = com.aimir.fep.bypass.dlms.DLMSClient.createObject(type);
   ...
   */
   public void capture()
   {
       synchronized (this)
       {
           ...
           int pos = -1;
           for (AbstractMap.SimpleEntry<DLMSObject, DLMSCaptureObject> obj : m_CaptureObjects)
            {
