...
public class BypassEncoder implements ProtocolEncoder {
	...
	public void encode(IoSession session, Object message,
		...
		if(message instanceof GeneralDataFrame)
        {
            ...
            buff.put(crc);
            buff.flip();
            log.info("GeneralDataFrame Sended : ["+session.getRemoteAddress()
                    +"] " + buff.limit() + " : "
                    + buff.getHexDump());
            out.write(buff);
        ...
        { 
            ...
            log.info("IoBuffer Sended : ["+session.getRemoteAddress()
                    +"] " + buffer.limit() + " : "
                    + buffer.getHexDump());
            out.write(buffer); 
        }
        else if(message instanceof byte[])
        {
            ...
            buffer.put(bx);
            buffer.flip();
            log.info("byte[] Sended : ["+session.getRemoteAddress()
                    +"] " + buffer.limit() + " : "
                    + buffer.getHexDump());
            out.write(buffer); 
        }
