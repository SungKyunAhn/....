...
{
    List<DLMSObjectDefinition> RegisterAssignment = new ArrayList<DLMSObjectDefinition>();           
    ...
    */
    public DLMSRegisterActivation(String ln)
    {
    ...
    */
    public DLMSRegisterActivation(String ln, int sn)
    {
    ...
    */
     ...
     * Add register.
     */
    void addRegister(DLMSObjectDefinition item)
    {
    ...
    public Object getValue(int index, int selector, Object parameters)
    {
        ...
        if (index == 2)
        {
            ...
            try
            {
                for(DLMSObjectDefinition it : getRegisterAssignment())
                {            
                    data.write((byte)DataType.STRUCTURE.getValue());
                    data.write(2);                
                    Common.setData(data, DataType.UINT16, it.getClassId().getValue());
                    Common.setData(data, DataType.OCTET_STRING, it.getLogicalName());
                }
            }
            catch(Exception ex)
            {
        ...
        if (index == 3)
        {
            ...
            for(AbstractMap.SimpleEntry<byte[], byte[]> it : MaskList)
            {
                data.write((byte)DataType.STRUCTURE.getValue());
                data.write(2);
                Common.setData(data, DataType.OCTET_STRING, it.getKey());
                data.write((byte)DataType.ARRAY.getValue());
                data.write((byte)it.getValue().length);
                for (byte b : it.getValue())
                {
                    Common.setData(data, DataType.UINT8, b);
                }
            }
            return data.toByteArray();
        }
    ...
    public void setValue(int index, Object value)
    {
        ...
        else if (index == 2)
        {
            ...
            if (value != null)
            {
                ...
                {
                    DLMSObjectDefinition item = new DLMSObjectDefinition();
                    item.setClassId(ObjectType.forValue(((Number)Array.get(it, 0)).intValue()));
                    item.setLogicalName(DLMSObject.toLogicalName((byte[]) Array.get(it, 1)));
                    RegisterAssignment.add(item);
                }
            }            
        }
