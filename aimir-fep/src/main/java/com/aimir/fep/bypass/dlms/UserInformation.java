...
class UserInformation {    
    ...
    void codeData(java.nio.ByteBuffer data)
    {
        ...
        data.put((byte)Common.InitialRequest); // Tag for xDLMS-Initiate request
    ...
    void encodeData(java.nio.ByteBuffer data)
    {
        ...
        tag = Common.unsignedByteToInt(data.get());
        boolean response = tag == Common.InitialResponce;
        if (response)
        {
        ...
        else if (tag == Common.InitialRequest)
        {
            //Optional usage field of the negotiated quality of service component
            tag = data.get();
            if (tag != 0) //Skip if used.
        ...
        //Old Way...
        if (Common.unsignedByteToInt(data.get(data.position())) == 0x1F)
        {
            data.get();
        }
        //Get len
        Common.unsignedByteToInt(data.get());
