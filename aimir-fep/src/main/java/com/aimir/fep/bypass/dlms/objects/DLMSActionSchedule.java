...
public class DLMSActionSchedule extends DLMSObject implements DLMSBase
{
    ...
    */
    public DLMSActionSchedule(String ln)
    {
    ...
    */
    public DLMSActionSchedule(String ln, int sn)
    {
    ...
    public final void setExecutionTime(DLMSDateTime[] value)
    {
    ...
    public Object getValue(int index, int selector, Object parameters)
    {
        ...
        if (index == 2)
        {
            ...
            try
            {
                Common.setData(stream, DataType.OCTET_STRING, getExecutedScriptLogicalName().getBytes("ASCII"));
                Common.setData(stream, DataType.UINT16, getExecutedScriptSelector());
            }            
            catch(Exception ex)
            {
        ...
        if (index == 4)
        {
            ...
            {
                Common.setObjectCount(0, stream);
            }
            else
            {
                Common.setObjectCount(getExecutionTime().length, stream);
                try
                {
                    for (DLMSDateTime it : getExecutionTime())
                    {
                        stream.write((byte)DataType.STRUCTURE.getValue());
                        stream.write((byte)2); //Count
                        Common.setData(stream, DataType.TIME, it.getValue()); //Time
                        Common.setData(stream, DataType.DATE, it.getValue()); //Date
                    }
                }
                catch(Exception ex)
                {
    ...
    public void setValue(int index, Object value)
    {
        ...
        {                
            setExecutedScriptLogicalName(DLMSClient.changeType((byte[])Array.get(value, 0), DataType.OCTET_STRING).toString());
        ...
        else if (index == 4)
        {
            ...
            if (value != null)
            {
                java.util.ArrayList<DLMSDateTime> items = new java.util.ArrayList<DLMSDateTime>();                
                for (Object it : (Object[])value)
                {
                    DLMSDateTime dt = (DLMSDateTime) DLMSClient.changeType((byte[])Array.get(it, 0), DataType.TIME);
                    DLMSDateTime dt2 = (DLMSDateTime) DLMSClient.changeType((byte[])Array.get(it, 1), DataType.DATE);                    
                ...
                setExecutionTime(items.toArray(new DLMSDateTime[items.size()]));
            }
        }                       
        else
        {
