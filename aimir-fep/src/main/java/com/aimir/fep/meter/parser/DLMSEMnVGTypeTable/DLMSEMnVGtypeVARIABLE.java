...
public class DLMSEMnVGtypeVARIABLE {
	private static Logger log = LoggerFactory.getLogger(DLMSEMnVGtypeVARIABLE.class);
	...
	public enum OBIS {
		...
		AVAILABLE_LP_COUNT("0100000101FF", "Available LP Count"), // 사용가능한 검침 자료 갯수
		REGULAR_LP_DATE("00000F0000FF", "Regular LP Date"), // 정기 검침일
		NON_REGULAR_LP_DATE("00000F0100FF", "Non-Regular LP Date"), // 비정기 검침일
		LPTIME_CURRENT_MONTH("0100000102FF", "LP Time Current Month"), // 현월 검침일
		LPTIME_LAST_MONTH("0100000102VZ", "LP Time Last Month"), // 전월 검침일   *이부분은 확인해볼것
		LP_INTERVAL("0101000804FF", "LpInterval"), // Load Profile 기록간격
		METER_CONSTANT_ACTIVE("0101000300FF", "Active POwer Constant"), // 유효전력량 계기정수
		METER_CONSTANT_REACTIVE("0101000301FF", "Non Active POwer Constant"), // 무효전력량 계기정수
		APPARENT_POWER_CONSTANT("0101000302FF", "Apparent Power Constant"), // 피상전력량 계기정수
		...
		BATTERY_INSTALL_TIME("0000600605FF", "Battery Install Time"), // 배터리 설치 일자/시간
		MONTHLY_ENERGY_PROFILE_CURRENT("0000620101FF", "Current Monthly Energy Profile"), //순방향 전력량 (현월)
		MONTHLY_ENERGY_PROFILE_LAST("0000620101VZ", "Last Monthly Energy Profile"), //순방향 전력량 (전월)
		MONTHLY_DEMAND_PROFILE_CURRENT("0000620102FF", "Current Monthly Demand Profile"), // 순방향 최대 수요전력(현월)
		MONTHLY_DEMAND_PROFILE_LAST("0000620102VZ", "Last Monthly Demand Profile"), // 순방향 최대 수요전력(전월)
		R_MONTHLY_ENERGY_PROFILE_CURRENT("0000620103FF", "Revers Current Monthly Energy Profile"), //역방향 전력량 (현월)
		R_MONTHLY_ENERGY_PROFILE_LAST("0000620103VZ", "Revers Last Monthly Energy Profile"), //역방향 전력량 (전월)
		...
		//    	LPTIME_CURRENT_MONTH("0100000102FF", "LP Time Current Month"), // 검침일자 현월
		PROGRAM_ID("0100000200FF", "Program Id"), //프로그램 ID //FW_VERSION("0100000200FF","Firmware Version"),
		//    	LP_INTERVAL("0101000804FF", "LpInterval"),   // Load Profile 기록간격
		LOAD_PROFILE("0100630100FF", "Load Profile"),
		A_MAX_LOAD_ELECTRIC("01001F2600FF", "A Maximum Load"), //A상 최대부하전류
		B_MAX_LOAD_ELECTRIC("0100332600FF", "B Maximum Load"), //B상 최대부하전류
		C_MAX_LOAD_ELECTRIC("0100472600FF", "C Maximum Load"), //C상 최대부하전류
		DEFAULT_VOLTAGE_AB("0100208000FF", "Default Voltage A_B"), //  평균전압 A-B상
		DEFAULT_VOLTAGE_BA("0100348000FF", "Default Voltage B_C"), //  평균전압 B-C상
		DEFAULT_VOLTAGE_CA("0100488000FF", "Default Voltage C_A"), //  평균전압 C-A상
		TEMPERATURE("0000600900FF", "Current Temperature"), // 현재온도
		OVERLAP_LP_ENTRY_NUMBER("010080000BFF", "Overlap Lp Entry Number"), // 최대 엔트리 수 이후에 오버랩되어 저장되는 LP엔트리 번호.
		//    	SELF_CHECK_BATTERY("0000616100FF", "Self Check Battery"),   // 자기진단1[배터리 이상] Data
		//    	SELF_CHECK_MEMORY("0000616101FF", "Self Check Memory"),     // 자기진단2[메모리 이상] Data
		//    	SELF_CHECK_POWER("0000616102FF", "Self Check Power"),       // 자기진단3[전압결상 등] Data
		MAGNETIC_SENSING_COUNT("0100800A00FF", "Magnetic Sensing Count"), // 자계 감지횟수
		SAG_COUNT("01000C2000FF", "SAG Count"), // SAG 발생횟수
		SWELL_COUNT("01000C2400FF", "Swell Count"), // Swell 발생횟수
		TERMINAL_COVER_OPEN_COUNT("008063620EFF", "Terminal Cover Open Count"); //터미널 커버 Open 횟수
...
//		MeterKind(2, "Meter Kind"), VendorNum(3, "Vendor"), MeterId(4, "Meter ID"), MeterDate(5, "Meter Date Time"), MeterStatusError(6, "Meter Error Status"), MeterStatusCaution(7, "Meter Caution Status"), MeterPulseConstant(8, "Meter Pulse Constant"), PTRatio(9, "PT Ratio"), CTRatio(10, "CT Ratio"), RegularReadDate(11, "Regular Read Date"), LoadProfileInterval(12, "Load Profile Interval"), RecentReadLoadProfileDate(
//				13, "Recent Read Load Profile Date"), LastReadInfo(14, "Last Read Information");
//
//		private int code;
//		private String name;
//
...
//	}
	...
	public enum MONTHLY_RE_ENERGY_PROFILE {
		...
		RE_ApparentEnergy(3, "Monthly Apparent Energy"), // 피상
		RE_LaggingReactiveEnergy(4, "Monthly Lagging Reactive Energy"), // 지상
		RE_LeadingReactiveEnergy(5, "Monthly Leading Reactive Energy"), // 진상
		...
		MONTHLY_RE_ENERGY_PROFILE(int code, String name) {
			this.code = code;
			this.name = name;
		}
...
//		ExportApparentEnergy(10, "Export Apparent Energy QII+QIII");     // 역방향 피상전력량
	...
	public enum DLMS_CLASS {
		DATA(1), REGISTER(3), EXTENDED_REGISTER(4),
		//        DEMAND_REGISTER(5),
		PROFILE_GENERIC(7), CLOCK(8),
		//      SCRIPT_TABLE(9);
	...
	public enum DLMS_CLASS_ATTR {
		...
		EXTENDED_REGISTER_ATTR05(5), // capture_time, octet-string    : 시간
		...
		PROFILE_GENERIC_ATTR08(8), // profile_entries, doublie-long-unsigned : 최대 저장개수
