 ...
 */
@Service
public class EMnVCompensatorSaver extends AbstractMDSaver {
	...
	public boolean save(IMeasurementData md) throws Exception {
		try {
			...
			if (md.getMeterDataParser() instanceof EMnVCompensator) {
				parser = (EMnVCompensator) md.getMeterDataParser();
				lplist = ((EMnVCompensator) parser).getLPData();
				meterId = ((EMnVCompensator) parser).getMeterID();
			} 
			...
			if (lplist == null || lplist.length < 1) {
				...
				try {
					...
					if (meter.getLastReadDate() == null || dsttime.compareTo(meter.getLastReadDate()) > 0) {
						...
						log.debug("##### [COMPENSATOR] 임시 통신시간 저장 체크 [미터={}] [dsttime-{} / 현재시간-{}", new Object[] { meter.getMdsId(), dsttime, notTime });
						log.debug("##### [COMPENSATOR] 임시 통신시간 저장 체크 [미터={}] [dsttime-{} / 현재시간-{}", new Object[] { meter.getMdsId(), dsttime, notTime });
						log.debug("##### [COMPENSATOR] 임시 통신시간 저장 체크 [미터={}] [dsttime-{} / 현재시간-{}", new Object[] { meter.getMdsId(), dsttime, notTime });
						...
						if (meter.getModem() != null) {
							...
							log.debug("##### [COMPENSATOR]모뎀 마지막 연결시간 저장 체크 [모뎀={}] [dsttime-{} / 현재시간-{}", new Object[] { tt, dsttime, DateTimeUtil.getCurrentDateTimeByFormat("yyyyMMddHHmmss").trim() });
							log.debug("##### [COMPENSATOR]모뎀 마지막 연결시간 저장 체크 [모뎀={}] [dsttime-{} / 현재시간-{}", new Object[] { tt, dsttime, DateTimeUtil.getCurrentDateTimeByFormat("yyyyMMddHHmmss").trim() });
							log.debug("##### [COMPENSATOR]모뎀 마지막 연결시간 저장 체크 [모뎀={}] [dsttime-{} / 현재시간-{}", new Object[] { tt, dsttime, DateTimeUtil.getCurrentDateTimeByFormat("yyyyMMddHHmmss").trim() });
						}
					}
