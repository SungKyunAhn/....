...
/**
 * DisplayItemSetting에 맞는 csv파일을 key, value 로 이루어진 Map새성.
 * @author jiae
 *
 */
public class DisplayItemSettingCSV implements java.io.Serializable, ConfigFile {
	...
	public DisplayItemSettingCSV(byte[] fileBinary) throws IOException {
		...
		if(str.length()>0){
				...
				if(split.length==2){
					...
					element.put(split[0], split[1] + "," + split[2]);
					isPreLiner=false;
				} else if(split.length>3){
					if(isPreLiner){
						head = split;
						...
						for(int i=0;i<split.length;i++ ){
							if(i>4) {
								m.put(element.keygen(head[head.length-1]),  m.get(element.keygen(head[head.length-1]))+ "," + split[i]);
							} else {
