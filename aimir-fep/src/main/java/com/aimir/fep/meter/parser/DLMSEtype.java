...
public class DLMSEtype extends MeterDataParser implements java.io.Serializable {
            ...
            if (key.equals(OBIS.TIME.getCode())) {
                ...
                if (resultSubData != null) {
                    ...
                    for (Iterator subi = resultSubData.keySet().iterator(); subi.hasNext();) {
                        ...
                        if (!subkey.contains(DLMSEtypeVARIABLE.UNDEFINED)) {
    ...
    public void parse(byte[] data) throws Exception {
        ...
        while ((pos+OBIS.length) < data.length) {
            ...
            if (dlms.getDlmsHeader().getObis() == DLMSEtypeVARIABLE.OBIS.LOAD_PROFILE) {
                for (int cnt = 0; ;cnt++) {
            ...
            for (int i = 0; i < result.size(); i++) {
                ...
                while ((value=lpMap.get(LOAD_PROFILE.ImportActive.name()+"-"+cnt)) != null) {
                    ...
                    _lpData = new LPData((String) lpMap
                            .get(LOAD_PROFILE.Time.name() + "-" + (cnt++)), lp, lpValue);
                    ...
                    if (value != null) {
                        if (value instanceof Float)
                            status = Hex.decode(DataUtil.get4ByteToInt(((Float)value).longValue()));
                        else if (value instanceof Long)
                            status = Hex.decode(DataUtil.get4ByteToInt((Long)value));
                    }
